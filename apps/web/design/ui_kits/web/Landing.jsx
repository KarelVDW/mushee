...
function Landing({ onSignIn, onGetStarted }) {
  return (
    <div data-screen-label="Landing" style={{ background: 'var(--color-surface)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
  ...
  return (
    <div role="dialog" aria-label="Cookie preferences"
      ...
      <div style={{ flex: 1, minWidth: 220, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <span style={{ font: '600 11px/1 var(--font-label)', letterSpacing: '0.14em',
        ...
        <p style={{ font: '400 14px/1.5 var(--font-body)', color: 'var(--color-on-surface)', margin: 0 }}>
          ...
          {' '}
          <a href="#" style={{ color: 'var(--color-primary)', textDecoration: 'underline' }}>Read more</a>.
        </p>
      </div>
...
function LandingNav({ onSignIn, onGetStarted }) {
  return (
    <nav style={{
      ...
      background: 'rgba(246,246,246,0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
    }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 32px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
          ...
          <PrimaryButton emphasis="pop" icon="arrow-right" onClick={onGetStarted}>Start composing</PrimaryButton>
        </div>
      </div>
    </nav>
  );
...
const navLinkStyle = {
  font: '500 14px/1 var(--font-body)', color: 'var(--color-on-surface-variant)',
  ...
  return (
    <section style={{ position: 'relative', overflow: 'hidden', paddingTop: 64, paddingBottom: 80 }}>
      <div style={{
        ...
        background: 'rgba(0,219,233,0.18)', borderRadius: '50%', filter: 'blur(120px)', pointerEvents: 'none',
      }} />
      <div style={{
        position: 'absolute', bottom: '-15%', left: '-5%', width: 360, height: 360,
        background: 'rgba(255,32,121,0.10)', borderRadius: '50%', filter: 'blur(120px)', pointerEvents: 'none',
      }} />
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 32px',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <h1 style={{
            fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 76, lineHeight: 0.95,
            letterSpacing: '-0.04em', color: 'var(--color-on-surface)', margin: 0,
          }}>Write the music<br/><em style={{ fontFamily: 'var(--font-serif)', fontWeight: 400 }}>in your head.</em></h1>
          <p style={{ font: '400 18px/1.5 var(--font-body)', color: 'var(--color-on-surface-variant)', margin: 0, maxWidth: 480 }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginTop: 8, flexWrap: 'wrap' }}>
            <PrimaryButton size="lg" emphasis="pop" icon="arrow-right" onClick={onGetStarted}>Start composing — it's free</PrimaryButton>
        ...
        <div style={{
          ...
          <div style={{ background: 'var(--color-surface-container-low)', borderRadius: 6, padding: 12, marginBottom: 12,
            display: 'flex', gap: 6 }}>
            {['𝅝','𝅗𝅥','𝅘𝅥','𝅘𝅥𝅮','𝅘𝅥𝅯'].map((g, i) => (
              <span key={i} style={{
                ...
                padding: '4px 10px', borderRadius: 4, fontSize: 16,
              }}>{g}</span>
            ))}
          </div>
...
function TrustStrip() {
  return (
    <section style={{ padding: '32px 32px', borderTop: '1px solid var(--color-outline-variant)', borderBottom: '1px solid var(--color-outline-variant)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap' }}>
        <span style={{ font: '600 11px/1 var(--font-label)', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--color-on-surface-variant)', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap', alignItems: 'center', opacity: 0.55 }}>
          {['Royal Conservatory', 'Berklee', 'Juilliard', 'RNCM', 'CalArts', 'Trinity Laban'].map(name => (
            <span key={name} style={{ font: '600 16px/1 var(--font-display)', letterSpacing: '-0.02em', color: 'var(--color-on-surface)' }}>{name}</span>
          ))}
        </div>
      </div>
    </section>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{
            ...
            letterSpacing: '-0.03em', color: 'var(--color-on-surface)', margin: '12px 0 0', maxWidth: 640,
          }}>From idea to score in three steps.</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32 }}>
          {steps.map(([num, title, body]) => (
            <div key={num} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <h3 style={{ font: '600 22px/1.25 var(--font-headline)', letterSpacing: '-0.01em', color: 'var(--color-on-surface)', margin: 0 }}>{title}</h3>
              <p style={{ font: '400 15px/1.55 var(--font-body)', color: 'var(--color-on-surface-variant)', margin: 0 }}>{body}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="features" style={{ padding: '88px 32px', background: 'var(--color-surface-container-lowest)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontStyle: 'italic',
          fontSize: 44, lineHeight: 1, letterSpacing: '-0.03em', margin: 0, marginBottom: 12, color: 'var(--color-on-surface)' }}>
          Quiet tools, real notation.
        </h2>
        <p style={{ font: '400 16px/1.5 var(--font-body)', color: 'var(--color-on-surface-variant)', maxWidth: 560, margin: 0, marginBottom: 48 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {[
            ...
            <div key={title} style={{
              ...
              <h3 style={{ font: '600 18px/1.3 var(--font-headline)', letterSpacing: '-0.005em', color: 'var(--color-on-surface)', margin: 0 }}>{title}</h3>
              <p style={{ font: '400 14px/1.55 var(--font-body)', color: 'var(--color-on-surface-variant)', margin: 0 }}>{body}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{
            fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 44, lineHeight: 1,
            letterSpacing: '-0.03em', color: 'var(--color-on-surface)', margin: '12px 0 0', maxWidth: 640,
          }}>People who write a lot of notes.</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {items.map((t, i) => (
            <figure key={i} style={{
              background: 'var(--color-surface-container-lowest)', borderRadius: 12, padding: 28,
              display: 'flex', flexDirection: 'column', gap: 20, margin: 0,
            }}>
              <span aria-hidden style={{
                font: '700 56px/0.7 var(--font-serif)', color: 'var(--color-primary)',
              }}>"</span>
              <blockquote style={{
                font: '400 17px/1.45 var(--font-serif)', fontStyle: 'italic',
              ...
              <figcaption style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 'auto' }}>
                ...
                }}>{t.name.split(' ').map(s => s[0]).join('')}</span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <span style={{ font: '600 14px/1.2 var(--font-body)', color: 'var(--color-on-surface)' }}>{t.name}</span>
                  <span style={{ font: '400 12px/1.2 var(--font-body)', color: 'var(--color-on-surface-variant)' }}>{t.role}</span>
                </div>
              </figcaption>
            </figure>
          ))}
...
function PricingTeaser({ onGetStarted }) {
  const tiers = [
    {
      name: 'Tinkerer', price: 'Free', sub: 'forever',
      bullets: ['30 sec recording / day', 'Up to 3 active scores', 'Real-instrument playback', 'Export to PDF & MusicXML'],
      cta: 'Start tinkering', emphasis: false,
    },
    {
      name: 'Hobbyist', price: '$15', sub: '/ month',
      bullets: ['10 min recording / day', 'Unlimited scores', 'Full instrument library', 'Lossless audio export', 'Priority support'],
      cta: 'Try Hobbyist free', emphasis: true,
    },
    {
      name: 'Professional', price: '$99', sub: '/ month',
      bullets: ['Unlimited recording', 'Everything in Hobbyist', 'Engraver-grade layout controls', 'Parts & score generation', 'Collaborator seats'],
      cta: 'Go Professional', emphasis: false,
    },
  ];
  return (
    <section id="pricing" style={{ padding: '88px 32px', background: 'var(--color-surface-container-lowest)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ marginBottom: 48, textAlign: 'center' }}>
          ...
          <h2 style={{
            fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 44, lineHeight: 1,
            letterSpacing: '-0.03em', color: 'var(--color-on-surface)', margin: '12px auto 0',
          }}>Pick the size that fits.</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {tiers.map(t => (
            <div key={t.name} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                {t.emphasis && (
                  <span style={{
                    font: '600 10px/1 var(--font-label)', textTransform: 'uppercase', letterSpacing: '0.12em',
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 40, letterSpacing: '-0.03em' }}>{t.price}</span>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                {t.bullets.map(b => (
                  <li key={b} style={{ display: 'flex', gap: 10, alignItems: 'flex-start',
                    ...
                    <Icon name="check" size={16} color={t.emphasis ? 'var(--color-tertiary)' : 'var(--color-primary)'} />
                    <span>{b}</span>
                  </li>
                ))}
              </ul>
              <div style={{ marginTop: 'auto', paddingTop: 8 }}>
                {t.emphasis
                  ? <PrimaryButton emphasis="pop" onClick={onGetStarted} style={{ width: '100%' }}>{t.cta}</PrimaryButton>
                  : <SecondaryButton onClick={onGetStarted} style={{ width: '100%' }}>{t.cta}</SecondaryButton>}
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <section style={{ padding: '96px 32px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 2,
        ...
        <h2 style={{
          fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 56, lineHeight: 1,
          letterSpacing: '-0.04em', color: 'var(--color-on-surface)', margin: 0,
