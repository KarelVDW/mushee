...
function AuthCard({ mode = 'signin', onSwitch, onSubmit }) {
  ...
  return (
    <main data-screen-label={`Auth · ${mode}`} style={{
      ...
      display: 'flex', overflow: 'hidden', minHeight: 580, position: 'relative',
    }}>
      {/* Decorative cyan haze */}
      <div style={{
        position: 'absolute', top: '-20%', right: '-10%', width: '50%', height: '50%',
        background: 'rgba(0,219,233,0.18)', borderRadius: '50%', filter: 'blur(96px)',
        pointerEvents: 'none',
      }} />
      {/* Brand panel */}
      <section style={{
        width: '42%', background: 'var(--color-surface-container-high)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          }}>{isSignup ? <>Compose<br/>without friction.</> : <>Welcome<br/>back.</>}</h1>
          <p style={{
            font: '400 14px/1.5 var(--font-body)', color: 'var(--color-on-surface-variant)',
            maxWidth: 280, margin: 0,
          }}>A simple, fast score editor for the music in your head.</p>
        </div>
      </section>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 360, width: '100%', margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 26,
            borderBottom: '1px solid rgba(172,173,173,0.2)', paddingBottom: 8, marginBottom: 32 }}>
            {[['signin','Sign in'], ['signup','Create account']].map(([m, label]) => (
              <button key={m} onClick={() => onSwitch(m)} style={{
                ...
                marginBottom: -9, paddingBottom: 8, whiteSpace: 'nowrap', flexShrink: 0,
              }}>{label}</button>
            ))}
          </div>
...
function PasswordResetCard({ onBackToSignIn }) {
  ...
  return (
    <main data-screen-label={`Auth · reset · ${stage}`} style={{
      width: '100%', maxWidth: 920, margin: '0 auto',
      background: 'var(--color-surface-container-lowest)', borderRadius: 16,
      boxShadow: '0 0 19px 0 rgba(45,47,47,0.06)',
      display: 'flex', overflow: 'hidden', minHeight: 580, position: 'relative',
      ...
      <section style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <h1 style={{
            ...
            fontSize: 48, lineHeight: 1, letterSpacing: '-0.02em',
            color: 'var(--color-on-surface)', margin: 0,
          }}>{stage === 'done' ? <>You're<br/>back in.</> : <>Reset your<br/>password.</>}</h1>
          <p style={{
            font: '400 14px/1.5 var(--font-body)', color: 'var(--color-on-surface-variant)',
            maxWidth: 280, margin: 0,
          }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 360, width: '100%', margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {STAGES.map((s, i) => (
              <button key={s} type="button"
                ...
                style={{
                  height: 4, flex: 1, borderRadius: 4, border: 0, padding: 0,
                  background: i <= stageIdx ? 'var(--color-primary-container)' : 'var(--color-surface-container)',
                  cursor: 'pointer',
                }} />
            ))}
            <Eyebrow style={{ marginLeft: 8 }}>{stageIdx + 1} of {STAGES.length}</Eyebrow>
          </div>

          {stage === 'request' && (
            <form onSubmit={(e) => { e.preventDefault(); if (email) setStage('sent'); }}
              style={{ display: 'flex', flexDirection: 'column', gap: 20, flex: 1 }}>
          ...
          {stage === 'sent' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, flex: 1 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, justifyContent: 'center' }}>
                ...
                <SubHeadline>
                  We sent a reset link to <strong style={{ color: 'var(--color-on-surface)' }}>{email || 'your address'}</strong>.
                ...
                <span style={{ font: '400 13px/1.5 var(--font-body)', color: 'var(--color-on-surface-variant)' }}>
                  ...
                  <button type="button" onClick={() => setResent(true)}
                    style={{ background: 'transparent', border: 0, padding: 0, color: 'var(--color-primary)', cursor: 'pointer', font: 'inherit', textDecoration: 'underline' }}>
          ...
          {stage === 'set-new' && (
            <form onSubmit={(e) => { e.preventDefault(); if (canSetNew) setStage('done'); }}
              style={{ display: 'flex', flexDirection: 'column', gap: 20, flex: 1 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, justifyContent: 'center' }}>
