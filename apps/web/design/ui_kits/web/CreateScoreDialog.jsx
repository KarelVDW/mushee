...
const ALL_INSTRUMENTS = [
  'Piano', 'Violin', 'Viola', 'Cello', 'Flute', 'Oboe', 'Bassoon', 'Clarinet',
  'Trumpet', 'Trombone', 'French horn', 'Tuba', 'Euphonium',
  'Harmonica', 'Recorder', 'Tin whistle', 'Pan flute', 'Ocarina',
  'Bagpipe', 'Erhu', 'Dizi flute', 'Shakuhachi', 'Guitar', 'Bass guitar', 'Harp',
];
...
function InstrumentPicker({ value, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minHeight: 0 }}>
      ...
      <div style={{
        flex: 1, minHeight: 160, overflowY: 'auto', display: 'flex', flexWrap: 'wrap',
        gap: 8, alignContent: 'flex-start', padding: '4px 0',
      }}>
        {filtered.map((i) => (
          <Chip key={i} active={i === value} onClick={() => onChange(i)}
            ariaLabel={`Pick ${i}`}>{i}</Chip>
        ))}
      </div>
    </div>
  );
...
function CreateScoreDialog({ onCancel, onCreate }) {
  ...
  return (
    <DialogScrim onDismiss={onCancel}>
      <DialogPanel
        title="New score" eyebrow="Give it a name and pick a lead instrument."
        onClose={onCancel} width={620}
        footer={<>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0, paddingBottom: 12 }}>
          <TextField label="Title" value={title} onChange={setTitle}
            placeholder="Untitled composition" autoFocus />
