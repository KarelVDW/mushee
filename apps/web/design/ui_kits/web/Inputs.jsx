...
function TextField({ label, value, onChange, type = 'text', placeholder, leftIcon, rightSlot, autoFocus, hint }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{
        background: 'var(--color-surface-container-low)', borderRadius: 4,
        position: 'relative', display: 'flex', alignItems: 'center', gap: 6,
        padding: leftIcon ? '0 10px 0 36px' : '0 10px',
      }}>
        {leftIcon && <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-outline)', display: 'inline-flex' }}>
        ...
        <input
          ...
          style={{
            ...
            color: 'var(--color-on-surface)', font: '400 14px/1 var(--font-body)',
            padding: '12px 4px', minWidth: 0,
          }} />
        {rightSlot}
        <div style={{
          position: 'absolute', left: focused ? 0 : '50%', right: focused ? 0 : '50%',
          bottom: 0, height: 2, background: 'var(--color-primary-container)',
      ...
      <div style={{ background: 'var(--color-surface-container-low)', borderRadius: 4, position: 'relative', padding: '0 10px' }}>
        <textarea value={value ?? ''} placeholder={placeholder} rows={rows}
          ...
          style={{
            ...
            color: 'var(--color-on-surface)', font: '400 14px/1.55 var(--font-body)',
