  ...
  return (
    <div role="group" aria-label={ariaLabel} style={{
      ...
      {options.map(([k, g]) => {
        ...
        return (
          <button key={k || 'default'} onClick={() => onChange(k)}
            style={{
    ...
    <button type="button" aria-label={ariaLabel} onClick={onClick}
      ...
      style={{
        background: active
          ...
          : hover ? 'var(--color-surface-container)' : 'var(--color-surface-container-low)',
        color: active ? 'var(--color-on-primary-container)' : 'var(--color-on-surface)',
        border: 0, borderRadius: 9999, padding: '7px 14px',
        font: '600 13px/1 var(--font-label)', cursor: 'pointer', whiteSpace: 'nowrap',
        minHeight: 32,
        transition: 'background 150ms var(--ease), color 150ms var(--ease)',
      }}>{children}</button>
  );
  ...
  if (tone === 'record') {
    ...
    return (
      <button type="button" aria-label={ariaLabel} onClick={onClick}
        onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
        style={{
          width: size, height: size, borderRadius: 9999, padding: 0,
...
function TempoPopover({ bpm, onChange, onClose }) {
  ...
  const commit = () => {
    ...
    if (n && n >= 20 && n <= 300) { onChange(n); onClose(); }
  };

  return (
    <div ref={popRef} role="dialog" aria-label="Set tempo"
      style={{
        position: 'absolute', top: 'calc(100% + 10px)', right: 0,
        background: 'var(--color-surface-container-lowest)',
        borderRadius: 12, padding: 16, width: 360, zIndex: 20,
      ...
      <button type="button" onClick={handleTap}
        style={{
          ...
          border: 0, borderRadius: 8, padding: '22px 16px',
          cursor: 'pointer', textAlign: 'left',
          display: 'flex', flexDirection: 'column', gap: 6,
        }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 8,
          font: '600 11px/1 var(--font-label)', letterSpacing: '0.14em',
        ...
        <span style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic',
          fontWeight: 700, fontSize: 20, lineHeight: 1.2, letterSpacing: '-0.02em' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 8 }}>
        <div style={{ position: 'relative', flex: 1, background: 'var(--color-surface-container-low)',
          borderRadius: 4, display: 'flex', alignItems: 'center', padding: '0 12px' }}>
          <input ref={inputRef} type="number" min={20} max={300}
            value={draft} onChange={(e) => setDraft(e.target.value)}
            aria-label="BPM"
            style={{
              flex: 1, background: 'transparent', border: 0, outline: 0,
              color: 'var(--color-on-surface)', font: '500 16px/1 var(--font-body)',
              padding: '12px 0', minWidth: 0,
            }} />
          <span style={{ font: '500 11px/1 var(--font-label)', letterSpacing: '0.12em',
            textTransform: 'uppercase', color: 'var(--color-on-surface-variant)' }}>bpm</span>
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 2,
            background: 'var(--color-primary-container)' }} />
        </div>
        <PrimaryButton onClick={commit} emphasis="pop">Set</PrimaryButton>
      </div>
    </div>
  );
}
...
function TempoBtn({ open, bpm, onClick }) {
  ...
  return (
    <button type="button" onClick={onClick}
      onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      aria-haspopup="dialog" aria-expanded={open} aria-label={`Tempo: ${bpm} bpm`}
      style={{
        background: open
          ? 'var(--color-primary-container)'
          : hover ? 'var(--color-surface-container)' : 'var(--color-surface-container-low)',
      ...
      <span style={{
        font: '600 10px/1 var(--font-label)', letterSpacing: '0.14em', textTransform: 'uppercase',
        opacity: 0.75,
      }}>bpm</span>
    </button>
  );
...
function RecordingUsage({ usedSec, limitSec, recording, planName, onClickWhenExhausted }) {
  ...
  return (
    <div role={exhausted ? 'button' : undefined}
      tabIndex={exhausted ? 0 : undefined}
      onClick={exhausted ? onClickWhenExhausted : undefined}
      onKeyDown={exhausted ? (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onClickWhenExhausted(); } } : undefined}
      style={{
      ...
      <span style={{
        ...
        {unlimited ? (
          ...
          <>
            <span style={{ color: recording ? 'var(--color-error)' : 'var(--color-on-surface)' }}>
              {fmtRecTime(usedSec)}
            </span>
      ...
      {!unlimited && (
        <div style={{
          width: 132, height: 3, borderRadius: 3, overflow: 'hidden',
          background: 'var(--color-surface-container)',
        }}>
          <div style={{
            width: `${pct * 100}%`, height: '100%',
            background: exhausted ? 'var(--color-error)'
              : recording ? 'var(--color-error-container)'
      ...
      <span style={{ font: '400 10px/1 var(--font-label)', letterSpacing: '0.08em', textTransform: 'uppercase',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 8 }}>
          ...
          {nextPlanName && (
            <span style={{ font: '400 13px/1.5 var(--font-body)', color: 'var(--color-on-surface-variant)' }}>
              Upgrading to <strong style={{ color: 'var(--color-on-surface)' }}>{nextPlanName}</strong>
              {' '}lifts the cap immediately and your remaining minutes carry over.
            </span>
          )}
        </div>
      </DialogPanel>
...
function ControlBar({ duration, onDuration, accidental, onAccidental, dotted, onDot, tied, onTie, playing, onPlay, onStop, recording, onRec, metro, onMetro, bpm, onBpm, recUsedSec, recLimitSec, recPlanName, onLimitClick }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', minWidth: 0 }}>
        <Segmented ariaLabel="Note duration" value={duration} onChange={onDuration} font="serif"
          options={[['w','𝅝'],['h','𝅗𝅥'],['q','𝅘𝅥'],['8','𝅘𝅥𝅮'],['16','𝅘𝅥𝅯']]} />
        <ChipToggle active={dotted} onClick={onDot} ariaLabel="Dotted">·</ChipToggle>
        <Segmented ariaLabel="Accidental" value={accidental} onChange={onAccidental}
          options={[['','♮'],['b','♭'],['#','♯']]} />
        <ChipToggle active={tied} onClick={onTie}>Tie</ChipToggle>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, position: 'relative' }}>
        ...
        <TransportBtn size={52} tone="play" active={playing} onClick={onPlay}
        ...
        <div style={{ position: 'relative', display: 'inline-flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <RecordingUsage
            usedSec={recUsedSec} limitSec={recLimitSec}
            recording={recording} planName={recPlanName}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', position: 'relative' }}>
        ...
        {tempoOpen && (
          <TempoPopover bpm={bpm} onChange={onBpm} onClose={() => setTempoOpen(false)} />
        )}
      </div>
    </div>
  );
...
function PlaceholderStaff() {
  ...
  return (
    <svg viewBox="0 0 800 220" width="100%" height="220" preserveAspectRatio="xMidYMid meet">
      {[0,1,2,3,4].map((i) => (
        <line key={i} x1={20} x2={780} y1={60 + i * lineGap} y2={60 + i * lineGap}
          stroke="#2d2f2f" strokeWidth={1} />
      ))}
      <text x={26} y={110} fontFamily="serif" fontSize={68} fill="#2d2f2f">𝄞</text>
      <text x={88} y={88} fontFamily="serif" fontSize={26} fontWeight="700" fill="#2d2f2f">4</text>
      <text x={88} y={108} fontFamily="serif" fontSize={26} fontWeight="700" fill="#2d2f2f">4</text>
      {[140, 220, 300, 380, 460, 540, 620, 700].map((x, i) => {
        ...
        return (
          <g key={x}>
            ...
            <line x1={stemUp ? x + 6 : x - 6} y1={cy} x2={stemUp ? x + 6 : x - 6} y2={cy + (stemUp ? -32 : 32)} stroke="#2d2f2f" strokeWidth={1.4} />
          </g>
        );
      })}
...
function Editor({ score, onBack, onSettings, planId = 'free', recUsedSec = 0, onRecUsedSecChange, onUpgrade }) {
  ...
  const handleRecClick = () => {
    if (exhausted) { setLimitDialogOpen(true); return; }
    setRecording((r) => !r);
  };

  return (
    <div data-screen-label="Editor"
      ...
      <header style={{
        ...
        boxShadow: '0 0 24px 0 rgba(45,47,47,0.06)', zIndex: 10,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, minWidth: 0, flex: 1 }}>
          <button onClick={onBack} aria-label="Back" style={{
            background: 'transparent', border: 0, cursor: 'pointer',
            color: 'var(--color-on-surface-variant)', padding: 4, display: 'inline-flex',
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)}
            style={{
              ...
              color: 'var(--color-on-surface)', padding: 4, minWidth: 0, flex: 1,
            }} />
          <Pill>{score?.instrument ?? 'Piano'}</Pill>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <SecondaryButton>Share</SecondaryButton>
          <PrimaryButton emphasis="pop" icon="check">Save</PrimaryButton>
        </div>
      </header>
      <ControlBar
        duration={duration} onDuration={setDuration}
        accidental={accidental} onAccidental={setAccidental}
        dotted={dotted} onDot={() => setDotted(!dotted)}
        tied={tied} onTie={() => setTied(!tied)}
        playing={playing} onPlay={() => setPlaying(!playing)}
        onStop={() => { setPlaying(false); setRecording(false); }}
        recording={recording} onRec={handleRecClick}
        metro={metro} onMetro={() => setMetro(!metro)}
        bpm={bpm} onBpm={setBpm}
        recUsedSec={recUsedSec} recLimitSec={limitSec}
      ...
      {limitDialogOpen && (
        <LimitReachedDialog
          ...
          onUpgrade={() => { setLimitDialogOpen(false); onUpgrade?.(); }} />
      )}
      <div style={{ flex: 1, overflow: 'auto', padding: 32 }}>
        <div style={{ maxWidth: 960, margin: '0 auto', minHeight: '100%',
          background: 'var(--color-surface-container-lowest)', padding: 40,
          boxShadow: '0 0 24px 0 rgba(45,47,47,0.06)' }}>
          <PlaceholderStaff />
          <PlaceholderStaff />
