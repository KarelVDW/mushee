  ...
  return (
    <div
      ...
      style={{
        ...
        display: 'grid', gridTemplateColumns: '5fr 2fr 2fr 2fr 1fr', gap: 16, alignItems: 'center',
        transition: 'background 220ms var(--ease)',
      }}>
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3,
        background: 'var(--color-primary-container)',
        opacity: hovered ? 1 : 0, transition: 'opacity 220ms var(--ease)' }} />
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <IconButton icon="pencil" ariaLabel="Edit" size={28} idleBg={hovered ? 'var(--color-surface-container-lowest)' : undefined} onClick={onOpen} />
        <IconButton icon="trash-2" ariaLabel="Delete" size={28} hoverTone="magenta" idleBg={hovered ? 'var(--color-surface-container-lowest)' : undefined} onClick={onDelete} />
      </div>
    </div>
  );
}
...
function Library({ scores, onOpen, onCreate, onDelete }) {
  ...
  return (
    <main data-screen-label="Library"
      style={{ flex: 1, maxWidth: 1280, margin: '0 auto', padding: '40px 32px',
        display: 'flex', flexDirection: 'column', gap: 24, width: '100%', boxSizing: 'border-box' }}>
      <PageHeader title="Your scores" italic
        subtitle="A quiet shelf for everything you're working on."
        right={<div style={{ width: 256 }}>
      ...
      {/* List header */}
      <div style={{
        display: 'grid', gridTemplateColumns: '5fr 2fr 2fr 2fr 1fr', gap: 16,
        padding: '8px 24px',
        font: '600 11px/1 var(--font-label)', letterSpacing: '0.1em', textTransform: 'uppercase',
        color: 'var(--color-outline)',
      }}>
        <span>Title</span><span>Instrument</span><span>Created</span><span>Updated</span><span></span>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {filtered.length === 0 ? (
          search ? (
            <div style={{
              background: 'var(--color-surface-container-lowest)', borderRadius: 8, padding: 56,
              boxShadow: '0 0 24px 0 rgba(45,47,47,0.06)', textAlign: 'center',
            ...
            <div style={{
              background: 'var(--color-surface-container-lowest)', borderRadius: 8,
              padding: '40px 32px',
              boxShadow: '0 0 24px 0 rgba(45,47,47,0.06)',
              ...
              <svg viewBox="0 0 120 80" width="96" height="64" aria-hidden style={{ flexShrink: 0 }}>
                {[0,1,2,3,4].map((i) => (
                  <line key={i} x1={8} x2={112} y1={20 + i * 10} y2={20 + i * 10}
                    stroke="var(--color-outline-variant)" strokeWidth={1} />
                ))}
                <text x={12} y={56} fontFamily="serif" fontSize={42}
              ...
              <PrimaryButton icon="plus" onClick={onCreate}>New score</PrimaryButton>
            </div>
          )
        ) : filtered.map((s, i) => (
