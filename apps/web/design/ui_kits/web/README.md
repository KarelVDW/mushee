...
- Score editor: slim header (back · title · instrument chip · centred transport · shortcuts · export) plus the bottom-docked note tool bar. Each tool group sits in a white capsule well (`ToolGroup` / the `Segmented` track — surface_container_lowest + tonal glow), controls transparent at rest; durations render as SVG note icons, then dot/tuplet/rest/tie, accidentals, clef, key signature, tempo, with upward-opening popovers. Sanctuary canvas with a static placeholder staff (real notation rendering lives in the `@mushee/notation` package, `packages/notation` — out of scope for the kit). Recording limit, concurrent-recording, and the iPhone Mic Mode guide dialogs, plus the returning-iPhone reminder toast.
- Settings (Profile / Account) with the change-plan grid, and the create-score dialog with the category-grouped instrument picker.
