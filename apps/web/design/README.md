...
- ALL-CAPS is used *only* for the small `--type-label-*` schematic eyebrows — short tags like "How it works" or "Pricing" — at `0.12em` tracking.
- Display headings use a hint of italic via Newsreader for warmth (e.g. "Write the music *in your head.*"); sans-serif Space Grotesk does the structural work.
...
- Avoid invented in-group nicknames ("operative", "maverick", "rebel"). Just call the user *you*.
...
**The sanctuary rule.** The score editor canvas is *strictly* `#ffffff` with `#2d2f2f` notes. **No cyan, no magenta in the editor area.** That space stays monochrome for cognitive focus.

**The three-intensity accent rule.** Each accent comes in three intensities — pick by the size *and* interactivity of the surface:
...
| **Newsreader** | Serif italic accent | The italic clause inside a display headline ("*in your head.*"), pull-quotes, testimonial copy. Optical-size variable. |
...
The wordmark is always **`Sheemu`** in Space Grotesk Bold (700) **italic**, tracked `-0.04em`. Lowercase save the capital S. This *is* the logo.
...
A Sheemu card is **`surface_container_lowest` + `--shadow-tonal` + `--radius-lg`**. *No border.* On hover, a list-style card may receive a left-side cyan accent stripe slide-in. Padding: `24–28px`. Cards never sit on each other directly — always with `0.8rem` (12.8px) of gap.
