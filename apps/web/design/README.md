...
**The sanctuary rule.** The score editor canvas is _strictly_ `#ffffff` with `#2d2f2f` ink (`NOTATION_INK`) — never pure black. **No cyan, no magenta in the editor area** — with two sanctioned exceptions, both transient and always excluded from exports: the live recording waveform's alternating cyan/magenta bars while a take runs, and the low-opacity magenta pitch-operation highlight (`HIGHLIGHT_MAGENTA`) that pulses under an open transpose popover's target range and flashes away when a transpose or minimize-accidentals lands. All other transient interaction overlays (selection bands, cursors) use the one sanctioned third color, `INTERACTION_BLUE` (`#1e90ff`), and are excluded from exports. Editor chrome is a slim header (transport centred) plus the docked **tool-dock** of note tools along the editor's bottom edge — the header's chrome mirror (`surface_container_low` at 85% + backdrop blur), never floating over the score; the canvas scrolls edge-to-edge between the two bars, tool groups wrap onto extra rows as they grow, and popovers open upward. On that chrome tone, controls take the card lift: each tool group sits in a white capsule well (`surface_container_lowest` + tonal glow — the `ToolGroup`/`Segmented` track) with its controls transparent at rest, and standalone chrome controls carry the white fill themselves.
