...
export class TieLayout {
    ...
    constructor(tie: Tie) {
        ...
        const row = tie.note.measure.score.getRowForMeasure(tie.note.measure)
        this.startX = row.layout.getMeasureX(tie.note.measure) + tie.note.layout.noteX + tie.note.width.noteHeadWidth
        this.startY = tie.note.layout.noteY + yShift
        this.endX = row.layout.getMeasureX(tie.nextNote.measure) +  tie.nextNote.layout.noteX
