...
The editor canvas is strictly `#ffffff` with **`#2d2f2f` ink — never pure black** (`NOTATION_INK` in `src/components/notation/constants.ts`). No cyan, no magenta inside the canvas — with two sanctioned exceptions, both transient by nature and always `data-export-exclude`d: the **live recording waveform** (`RecordingWaveform.tsx`) renders alternating cyan/magenta bars while a take is in flight, and the **pitch-operation highlight** (`HighlightLayer.tsx`, `HIGHLIGHT_MAGENTA`) washes the affected range in low-opacity magenta — pulsing under an open transpose popover, flashing away when a transpose or minimize-accidentals lands. All other transient interaction overlays (selection bands, input cursor, playback cursor) use the one sanctioned third color, `INTERACTION_BLUE` (`#1e90ff`) — deliberately neither brand accent, always `data-export-exclude`d so exports stay monochrome. The optional `manuscript-canvas` dot texture (40px grid, 15% `outline_variant`) may sit behind the staff.
...
- **The Tool-Dock:** the editor's note tools live in a docked bar along the editor's bottom edge — the chrome mirror of the slim header (`surface-container-low/85` + `backdrop-blur-xl`), never floating over the score. The canvas scrolls edge-to-edge between the two bars. Each tool group sits in a **white capsule well** (`surface_container_lowest` + tonal glow — the card lift, shared by the `Segmented` track and `ToolGroup`): the well is what says "buttons live here" and where one group ends, so groups need only 1.5rem of space between them — **no dividers**. Controls inside a well are transparent at rest (`plain`); controls standing alone on the chrome (header chips, transport circles) carry the white fill themselves. New tool groups wrap onto additional rows rather than overflowing. Popovers open upward, away from the dock. The transport (stop / play / record / metronome) sits centred in the slim editor header.
...
- **Selection actions live in the selection popover, never the dock.** Long-press, double-tap, or lifting a range drag opens the floating selection bar (`SelectionPopover` — deliberately OS-flavored, mirroring Android's text-selection menu) with copy / paste / delete / select all. Selection-scoped actions are never added as dedicated dock or action-row buttons.
- **Dock popovers become sheets.** Clef / key / time / tempo popovers span the dock's width (`popoverPosition(compact)`) instead of anchoring to their chip — anchored panels clip at the viewport edge. Tapping the same attributes inline in the score opens the identical sheet (`COMPACT_POPOVER_SHEET`); on desktop those in-score popovers anchor at the clicked glyph, clamped inside the score wrapper (`AttributePopoverAnchor`).
