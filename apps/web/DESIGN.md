...
- **The Tool-Dock:** the editor's note tools live in a floating glass dock (`glass-panel` + `rounded-lg`) hovering over the bottom of the score. Groups are separated by 1.5rem of space — **no dividers**. Popovers open upward, away from the dock. The transport (stop / play / record / metronome) sits centred in the slim editor header.
