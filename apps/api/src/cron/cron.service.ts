...
export class CronService {
    ...
    private async withCronLock(key: number, fn: () => Promise<void>): Promise<void> {
        ...
        try {
            const rows = (await runner.query('SELECT pg_try_advisory_lock($1) AS locked', [key])) as Array<{ locked: boolean }>
    ...
    async pruneProcessedWebhookEvents(): Promise<void> {
        await this.withCronLock(LOCK_PRUNE_WEBHOOKS, async () => {
            await this.dataSource.query(`DELETE FROM processed_webhook_events WHERE "receivedAt" < now() - interval '90 days'`)
        })
    }
    ...
    private async flushStaleScoresLocked(): Promise<void> {
        ...
        for (const cached of staleScores) {
            try {
                ...
                if (!score) {
                    this.logger.warn(`Score ${cached.scoreId} not found in DB, removing from cache`)
                ...
                await this.cacheService.deleteIfNotUpdatedSince(cached.scoreId, cached.updatedAt)

                this.logger.log(`Flushed score ${score.id} (${score.title}) to storage`)
            } catch (error) {
                this.logger.error(`Failed to flush score ${cached.scoreId}: ${String(error)}`)
            }
        }
    }
}
