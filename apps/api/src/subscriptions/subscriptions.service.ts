...
export class SubscriptionsService {
    private tierCache: { at: number; byId: Map<string, SubscriptionTier> } | null = null
    ...
    async tierById(id: string | null | undefined): Promise<SubscriptionTier> {
        ...
        if (!tier) {
            throw new Error('Subscription tier catalogue is empty — has the seed migration run?')
        }
        return tier
    }
    ...
    async findForUser(userId: string, manager?: EntityManager): Promise<UserSubscription | null> {
