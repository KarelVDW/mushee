...
const MAX_BODY_BYTES = parseInt(process.env.MAX_BODY_BYTES ?? String(5 * 1024 * 1024), 10)
...
async function bootstrap() {
    ...
    const jsonLogs = process.env.LOG_FORMAT === 'json' || (process.env.NODE_ENV === 'production' && process.env.LOG_FORMAT !== 'pretty')
    ...
    if (process.env.SEED_DEMO_DATA === 'true') {
        if (process.env.NODE_ENV === 'production') {
            throw new Error('SEED_DEMO_DATA must not be set in production: it creates accounts with a publicly known password')
        }
        await seedDemoData()
    }
