...
export class AccountService {
     ...
     */
    async requestDeletion(userId: string, password: string): Promise<AccountDeletionStatus> {
        const ctx = await auth.$context

        const accounts = await ctx.internalAdapter.findAccounts(userId)
        const credential = accounts.find((a) => a.providerId === 'credential')
        const valid = credential?.password && (await ctx.password.verify({ hash: credential.password, password }))
        if (!valid) {
        ...
        const deletion =
            ...
            (await this.deletionRepo.save(
                this.deletionRepo.create({
                    userId,
                    purgeAfter: new Date(Date.now() + DELETION_GRACE_PERIOD_DAYS * 24 * 60 * 60 * 1000),
                }),
            ))

        await ctx.internalAdapter.deleteSessions(userId)
        this.logger.log(`Account ${userId} scheduled for deletion after ${deletion.purgeAfter.toISOString()}`)
    ...
    async purgeExpired(): Promise<number> {
        ...
        for (const { userId } of due) {
            try {
                ...
                if (user?.email) {
                    await this.deletionRepo.manager.query(`DELETE FROM "verification" WHERE identifier LIKE '%' || $1 || '%'`, [user.email])
                }
