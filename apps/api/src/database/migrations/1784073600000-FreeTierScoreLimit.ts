...
export class FreeTierScoreLimit1784073600000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "subscription_tiers" ADD "maxScores" integer`)
        await queryRunner.query(`UPDATE "subscription_tiers" SET "maxScores" = 5 WHERE "id" = 'free'`)
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "subscription_tiers" DROP COLUMN "maxScores"`)
    }
}
