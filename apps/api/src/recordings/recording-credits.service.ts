...
export class RecordingCreditsService {
    ...
    async spend(userId: string, credits: number): Promise<RecordingCreditBalance> {
       ...
       RETURNING "creditsUsed"`,
        ...
        if (limit !== null && used >= limit) {
            ...
            packSeconds = overflow > 0 ? await this.drawPackSeconds(userId, overflow) : await this.packSecondsOf(userId)
        }
     ...
     * retries cleanly.
     */
    async grantPackSeconds(userId: string, seconds: number, manager?: EntityManager): Promise<void> {
     ...
     */
    async revokePackSeconds(userId: string, seconds: number, manager?: EntityManager): Promise<void> {
        const repo = manager?.getRepository(CreditBalance) ?? this.balanceRepo
        await repo.query(
    ...
    private async drawPackSeconds(userId: string, seconds: number): Promise<number> {
    ...
    private toBalance(tier: SubscriptionTier, used: number, packSeconds: number): RecordingCreditBalance {
        const limit = tier.dailyRecordingCredits
