 ...
 * stats catch drift).
 */
const DENOISE_FILTER = process.env.RECORDING_DENOISE_FILTER ?? 'afftdn=nr=12:nf=-40:tn=1,atrim=start=0.025'
...
function decodeArgs(filterChain: string, targetSampleRate: number, inputFormat?: string): string[] {
    return [
        '-hide_banner',
        '-loglevel',
        'error',
...
export class AudioDecoder {
    ...
    decode(buffer: Buffer, targetSampleRate: number, opts?: DecodeOptions): Promise<DecodedAudio> {
        ...
        return new Promise<DecodedAudio>((resolve, reject) => {
            const proc = spawn(this.ffmpeg, decodeArgs(filterChain, targetSampleRate, opts?.inputFormat))
            ...
            proc.on('close', (code) => {
                ...
                if (out.byteLength === 0) {
                    reject(new Error(`ffmpeg produced no audio (exit ${code}): ${err.trim() || 'no stderr'}`))
                    return
                }
                const samples = new Float32Array(out.buffer, out.byteOffset, out.byteLength / Float32Array.BYTES_PER_ELEMENT)
                resolve({
                    samples,
...
export class StreamingDecoder {
    ...
    constructor(
        ...
        const filterChain = buildFilterChain(opts?.highpassHz ?? 80, opts?.loudnorm ?? true, opts?.denoise ?? false)
        this.buf = new Float32Array(sampleRate * 8) // ~8 s initial, grows as needed
        this.proc = spawn(resolveFfmpeg(), decodeArgs(filterChain, sampleRate, opts?.inputFormat))
    ...
    private ingest(chunk: Buffer): void {
        ...
        this.carry = fullBytes === data.length ? EMPTY : Buffer.from(data.subarray(fullBytes))
