import type { MxmlMeasure, MxmlMeasureEntry, MxmlNoteType, MxmlPitch, MxmlStep, MxmlTie } from './mxml.types'
import { chooseNamingOffset, estimateGridPhaseBeats, estimateTuningOffsetCents, keyPitchClasses, spellMidi } from './voice-notation'
...
export class MxmlBuilder {
    ...
    measureRangeFor(startTimeSeconds: number, durationSeconds: number): [number, number] {
        ...
        const last = this.measureIndexFor(Math.max(startTimeSeconds, startTimeSeconds + durationSeconds - 1e-6))
    ...
    buildMeasure(index: number, allNotes: PendingNote[]): MxmlMeasure {
        ...
        const keyClasses = this.voiceSpelling && this.options.keyFifths != null ? keyPitchClasses(this.options.keyFifths) : null
        const offsetCents = this.voiceSpelling ? chooseNamingOffset(allNotes, estimateTuningOffsetCents(allNotes), keyClasses) : 0
        ...
        for (const n of allNotes) {
            ...
            segments.push({
                ...
                pitchMidi: this.voiceSpelling ? spellMidi(n, offsetCents, keyClasses) : n.pitchMidi,
            })
        }
        ...
        for (const seg of segments) {
            ...
            this.appendNoteSegments(entries, cursorBeat - measureStartBeat, span, seg.pitchMidi)
            cursorBeat += span
        }
        if (cursorBeat < measureEndBeat) {
            this.appendRests(entries, cursorBeat - measureStartBeat, measureEndBeat - cursorBeat)
        }

        return { number: String(index + 1), entries }
    }

    private appendRests(entries: MxmlMeasureEntry[], startBeatInMeasure: number, beats: number): void {
    ...
    private appendNoteSegments(entries: MxmlMeasureEntry[], startBeatInMeasure: number, beats: number, midi: number): void {
     ...
     */
    private spellDuration(startBeatInMeasure: number, beats: number): StandardDuration[] {
    ...
    private spellSpan(start: number, divs: number): StandardDuration[] {
        ...
        for (const level of this.boundaryLevels()) {
            ...
            if (next > start && next < start + divs) {
                return [...this.spellSpan(start, next - start), ...this.spellSpan(next, start + divs - next)]
            }
        }
