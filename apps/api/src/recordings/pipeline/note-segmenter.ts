...
export class NoteSegmenter {
    ...
    segment(track: PitchTrack, energy?: Float32Array): NoteEventTime[] {
        ...
        const longQuiet =
            this.dropLongQuiet && energyRef !== null && energy
                ? {
                      minFrames: Math.max(1, Math.round((this.dropLongQuiet.minSec ?? 0.35) / track.hopSec)),
     ...
     */
    private changeDiscount(frames: number, energy: Float32Array | undefined): Float32Array | null {
     ...
     */
    private medianMidi(track: PitchTrack, voiced: Uint8Array, run: Run): number | null {
        const vals: number[] = []
