...
export interface VoiceDecodeOptions {
     ...
     */
    pitchEstimator?: 'trimmed-mean' | 'hann-median' | 'slew-limit' | 'one-pole' | 'detrend' | 'slope-gated'
...
export class VoiceNoteDecoder {
    ...
    decode(track: PitchTrack, energy?: Float32Array): NoteEventTime[] {
        ...
        if (this.o.fillUnvoicedGapSec !== undefined) {
            track = track.fillDropouts({
                ...
                maxGapFrames: Math.max(1, Math.round(this.o.fillUnvoicedGapSec / track.hopSec)),
                energy,
                energyFloorRatio: this.o.fillEnergyFloor,
                energyContextFrames: this.o.fillEnergyContextSec === undefined ? 0 : Math.round(this.o.fillEnergyContextSec / track.hopSec),
            })
        }
        ...
        const { pitches, attack, reonset } = this.viterbi(track, voiced, grid, frames, evidence, accent)
        ...
        const longQuiet =
            this.o.dropLongQuiet && energyRef !== null && energy
                ? {
                      minFrames: Math.max(1, Math.round((this.o.dropLongQuiet.minSec ?? 0.35) / track.hopSec)),
                      floor: (this.o.dropLongQuiet.quietRatio ?? 0.3) * energyRef,
                  }
                : null
        const runs = this.absorbShortRuns(this.runsOf(pitches, frames, reonset), minFrames, keepShortLoud)

        const notes: NoteEventTime[] = []
        for (const run of runs) {
            if (run.state < 0) continue
            if (longQuiet && energy && run.end - run.start >= longQuiet.minFrames && meanOver(energy, run) < longQuiet.floor) {
                continue
            }
     ...
     */
    private pitchGrid(track: PitchTrack, voiced: Uint8Array): { centres: Float32Array; step: number } | null {
        let lo = Infinity
        let hi = -Infinity
     ...
     */
    private boundaryEvidence(track: PitchTrack, voiced: Uint8Array, energy: Float32Array | undefined): Float32Array | null {
        const frames = track.frames
     ...
     */
    private accentCurve(track: PitchTrack, energy: Float32Array | undefined): Float32Array | null {
    ...
    private viterbi(
        ...
        const wideGuard = Math.max(guard + 1, Math.round((this.o.wideIntervalSemitones * 100) / step))
        ...
        const icCapSteps = ic ? Math.max(1, Math.round(((ic.capSemitones ?? 13) * 100) / step)) : 0
        ...
        const ucrAfterFrames = ucr ? Math.max(1, Math.round((ucr.afterSec ?? 0.06) / track.hopSec)) : 0
            ...
            for (let p = 0; p < n; p += 1) {
                // attack(p): keep attacking, start from silence, or (direct mode) change note.
                {
                    ...
                    if (sm && silenceFromPitch >= 0) {
                        ...
                        fromSilence += (smPerOct * octaves) / (sm.amortize ? Math.max(1, silenceGapFrames) : 1)
                    }
                    if (fromSilence < best) {
                        best = fromSilence
                        from = 0
                    ...
                    if (Number.isFinite(this.o.reonsetCost)) {
                        ...
                        const reonset = cost[STABLE + p] + this.o.reonsetCost * ev - (accent ? this.o.accentBonus * accent[t] : 0)
                        if (reonset < best) {
                            best = reonset
                            from = STABLE + p
                        }
                    ...
                    if (!viaSilence) {
                        ...
                        const tryFrom = (lookup: number, side: 'left' | 'right', cost: number): void => {
                            const at =
                                side === 'left'
                                    ? lookup >= 1
                                        ? prefixAt[Math.min(lookup, n - 1)]
                                        : -1
                                    : lookup <= n - 2
                                      ? suffixAt[Math.max(lookup, 0)]
                                      : -1
                            const c =
                                side === 'left'
                                    ? lookup >= 1
                                        ? prefix[Math.min(lookup, n - 1)]
                                        : Infinity
                                    : lookup <= n - 2
                                      ? suffix[Math.max(lookup, 0)]
                                      : Infinity
                        ...
                        if (ic) {
                            ...
                            for (let q = lo; q <= hi; q += 1) {
                                ...
                                const shape = ic.form === 'linear' ? icPerOct * (dSemis / 12) : (dSemis * dSemis) / icTwoSigSq
     ...
     */
    private candidateTable(track: PitchTrack): { k: number; cents: Float32Array; strength: Float32Array; weakness: Float32Array } | null {
        ...
        for (let t = 0; t < frames; t += 1) {
            ...
            if (o.octaveBias !== undefined && list.length > 1) {
                ...
                for (let a = 0; a < list.length; a += 1) {
                    for (let b = 0; b < list.length; b += 1) {
                        ...
                        if (Math.abs(list[a].c - list[b].c - 1200) <= 50 && list[b].s < (o.octaveBias ?? 1) * list[a].s) {
                            drop.add(b)
                        }
                    }
                }
            ...
            for (let j = 0; j < kept.length; j += 1) {
                ...
                weakness[t * k + j] = yinTrust * -Math.log(Math.max(1e-6, kept[j].s / maxS))
            }
        }
     ...
     */
    private onsetFrameOf(run: Run, attack: Uint8Array, track: PitchTrack, centsOfNote: number): number {
        ...
        return this.o.onsetAt === 'mid' ? Math.floor((run.start + settled) / 2) : settled
    }
     ...
     */
    private absorbShortRuns(runs: Run[], minFrames: number, keepShortLoud?: (run: Run) => boolean): Run[] {
        let work = runs
     ...
     */
    private noteCents(track: PitchTrack, voiced: Uint8Array, run: Run): number | null {
     ...
     */
    private applyMergeGuard(notes: NoteEventTime[], track: PitchTrack, voiced: Uint8Array): NoteEventTime[] {
        ...
        for (let i = 1; i < notes.length; i += 1) {
            ...
            const merged = this.noteCents(track, voiced, {
                start: Math.max(0, Math.round((prev.startTimeSeconds - this.o.onsetShiftSec) / track.hopSec)),
                end: Math.min(track.frames, Math.round(mergedEnd / track.hopSec)),
                state: 0,
            })
            if (merged !== null) {
                prev.pitchMidi = Math.round(merged / 100)
                ;(prev as NoteEventTime & { pitchMidiFloat?: number }).pitchMidiFloat = merged / 100
            }
        }
        return out
    }
    ...
    private splitIsJustified(
        ...
        const from = Math.max(0, Math.round((cur.startTimeSeconds - this.o.onsetShiftSec) / track.hopSec))
        const to = Math.min(track.frames, Math.round((cur.startTimeSeconds + cur.durationSeconds) / track.hopSec))
 ...
 */
function medianVoicedEnergy(voiced: Uint8Array, frames: number, energy: Float32Array | undefined): number | null {
