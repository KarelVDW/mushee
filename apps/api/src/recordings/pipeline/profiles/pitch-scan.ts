...
export function scanPitch(samples: Float32Array, sampleRate: number): PitchScan {
    ...
    const maxBin = Math.min(FFT_SIZE / 2 - 1, Math.ceil((GLOBAL_MAX_FREQ_HZ * FFT_SIZE) / sampleRate))
    ...
    for (let start = 0; start + FFT_SIZE <= samples.length; start += HOP) {
        ...
        for (let seg = 0; seg < RESIDUAL_BANDS; seg += 1) {
            ...
            const hi = Math.min(residualMaxBin, Math.round(minBin * Math.pow(residualMaxBin / minBin, (seg + 1) / RESIDUAL_BANDS)) - 1)
            if (hi < lo) continue
            let ri = 0
