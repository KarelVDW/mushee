...
export class RecordingDebugRenderer {
    render(opts: DebugRenderOptions): string {
        ...
        const minPitch = (allNotes.length ? Math.min(...allNotes.map((n) => n.pitchMidi)) : 60) - PITCH_PADDING
        const maxPitch = (allNotes.length ? Math.max(...allNotes.map((n) => n.pitchMidi)) : 72) + PITCH_PADDING
        ...
        const yFor = (midi: number) => MARGIN_TOP + (maxPitch - midi) * pxPerSemi

        const elements: string[] = []

        // Background
        elements.push(`<rect x="0" y="0" width="${width}" height="${height}" fill="#fff"/>`)
