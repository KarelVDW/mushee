 ...
 */
export function viterbi(activations: Float32Array, frames: number, opts: ViterbiOptions): Int16Array {
    ...
    for (let t = 1; t < frames; t++) {
        ...
        for (let bCur = 0; bCur < numBins; bCur++) {
            ...
            for (let bPrev = lo; bPrev <= hi; bPrev++) {
                const s = logProb[offPrev + bPrev] + logTrans[bCur - bPrev + bandBins]
                if (s > bestScore) {
                    bestScore = s
                    bestPrev = bPrev
                }
            }
            logProb[offCur + bCur] = bestScore + Math.log(activations[offCur + bCur] + 1e-15)
 ...
 * current run.
 */
export function segmentNotes(cents: Float32Array, confidence: Float32Array, frames: number, opts: SegmentOptions): NoteEventTime[] {
    ...
    for (let i = 0; i < frames; i++) {
        ...
        const voiced = conf >= opts.confidenceThreshold && hz >= opts.minFreqHz && hz <= opts.maxFreqHz
        if (!voiced) {
            finalize(i)
            continue
        }
 ...
 */
function estimateTuningOffset(cents: Float32Array, frames: number, isVoiced: (i: number) => boolean): number {
    let sumSin = 0
    let sumCos = 0
    let n = 0
...
export function segmentNotesBySemitone(
    ...
    const isVoiced = (i: number): boolean => {
        ...
        return confidence[i] >= opts.confidenceThreshold && hz >= opts.minFreqHz && hz <= opts.maxFreqHz
    }
 ...
 */
export function frameEnergy(samples: Float32Array, sampleRate: number, hopSec: number, frames: number): Float32Array {
