...
export class RemoteModelBackend implements ModelBackend {
    ...
    constructor(
        ...
        const ServiceCtor = pkg['CrepeInference'] as new (address: string, creds: grpc.ChannelCredentials, options?: object) => GrpcClient
    ...
    async warm(model: ModelKey): Promise<void> {
        ...
        try {
            ...
            this.logger.warn(`Inference service for ${model} at ${this.url} not ready: ${describe(err)}`)
        }
    }

    async crepePredict(frames: Float32Array, batchCount: number): Promise<Float32Array> {
    ...
    private unary<R>(method: string, req: object): Promise<R> {
        const fn = this.client[method] as (req: object, opts: grpc.CallOptions, cb: (err: grpc.ServiceError | null, res: R) => void) => void
