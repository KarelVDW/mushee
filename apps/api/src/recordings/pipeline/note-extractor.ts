...
export class NoteExtractor {
     ...
     */
    private suppressTransients(notes: NoteEventTime[], bpm: number): NoteEventTime[] {
        ...
        for (let i = 0; i < notes.length; i += 1) {
            ...
            if (
                ...
                (cur.startTimeSeconds - (prev.startTimeSeconds + prev.durationSeconds)) * beatsPerSecond < maxGapBeats &&
                (next.startTimeSeconds - (cur.startTimeSeconds + cur.durationSeconds)) * beatsPerSecond < maxGapBeats
            ) {
     ...
     */
    private splitAtOnsets(notes: NoteEventTime[], onsetTimesSec: number[] | undefined): NoteEventTime[] {
        ...
        for (const note of notes) {
            ...
            const interior = onsetTimesSec.filter((t) => t > note.startTimeSeconds + MIN_SEGMENT_SEC && t < end - MIN_SEGMENT_SEC)
     ...
     */
    private selectMonophonic(notes: NoteEventTime[], bpm: number): NoteEventTime[] {
        const beatsPerSecond = bpm / 60
        const sorted = [...notes].sort(
            (a, b) => a.startTimeSeconds - b.startTimeSeconds || b.amplitude - a.amplitude || a.pitchMidi - b.pitchMidi,
        )
        ...
        for (const note of sorted) {
            ...
            if (note.amplitude > prev.amplitude * AMPLITUDE_OVERRIDE_RATIO) {
                prev.durationSeconds = Math.max(0, note.startTimeSeconds - prev.startTimeSeconds)
     ...
     */
    private mergeAdjacent(notes: NoteEventTime[], bpm: number): NoteEventTime[] {
        const beatsPerSecond = bpm / 60
        const result: NoteEventTime[] = []
        for (const note of notes) {
            const prev = result[result.length - 1]
            ...
            const shortestBeats = Math.min(prev.durationSeconds, note.durationSeconds) * beatsPerSecond
            if (
            ...
            const pairDeviation = this.biasDeviation(prev, beatsPerSecond) + this.biasDeviation(note, beatsPerSecond)
     ...
     */
    private biasDeviation(note: NoteEventTime, beatsPerSecond: number): number {
        ...
        const durDev = Math.abs(durationBeats - this.nearestStandardDuration(durationBeats))
     ...
     */
    private alignAndQuantize(notes: NoteEventTime[], bpm: number): NoteEventTime[] {
        const beatsPerSecond = bpm / 60
        const result: NoteEventTime[] = []
        ...
        for (let i = 0; i < notes.length; i++) {
            const note = notes[i]
            const startBeat = note.startTimeSeconds * beatsPerSecond
            const endBeat = (note.startTimeSeconds + note.durationSeconds) * beatsPerSecond
            ...
            if (
                ...
                durationBeats = this.snapToStandardDuration(Math.min(soundingBeats, maxAvailable))
            }
