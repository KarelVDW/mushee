...
import { RecordingArchiver, sniffContainer } from '../../src/recordings/recording-archiver'
...
describe('RecordingArchiver', () => {
    it('streams audio under the recording path with a sniffed extension', async () => {
        ...
        expect(createWriteStream).toHaveBeenCalledWith('recordings/u/s/r/audio.webm', { contentType: 'audio/webm' })
        expect(objects.get('recordings/u/s/r/audio.webm')).toEqual(Buffer.concat([WEBM_HEADER, Buffer.from('more-audio')]))
    })
    ...
    it('writes the debug bundle beside the audio', async () => {
        ...
        expect(objects.get('recordings/u/s/r/score.json')?.toString()).toBe('{"measures":{}}')
        expect(JSON.parse(objects.get('recordings/u/s/r/session.json')?.toString() ?? 'null')).toEqual({ bpm: 120 })
    })
    ...
    it('survives a failing audio stream and still archives the bundle', async () => {
        ...
        archiver.appendAudio(WEBM_HEADER)
        streams.get('recordings/u/s/r/audio.webm')?.destroy(new Error('bucket unreachable'))
