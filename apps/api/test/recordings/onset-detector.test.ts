...
function twoNotesWithGap(gapSec: number): Float32Array {
    const noteSec = 0.4
    return shaped(noteSec * 2 + gapSec, (t) => (t < noteSec || t >= noteSec + gapSec ? 0.5 : 0.0))
}
...
function saggingHeldNote(sagSec: number): Float32Array {
    const noteSec = 0.4
    return shaped(noteSec * 2 + sagSec, (t) => (t < noteSec || t >= noteSec + sagSec ? 0.5 : 0.1))
}
...
describe('OnsetDetector', () => {
    ...
    it('detectFromEnvelope reproduces detect() exactly', () => {
        ...
        expect(detector.detectFromEnvelope(detector.envelope(samples, SR), hop, SR)).toEqual(detector.detect(samples, SR))
    })
    ...
    it('minTroughSec of 0 is exactly the historical behaviour', () => {
        ...
        expect(new OnsetDetector({ minTroughSec: 0 }).detect(samples, SR)).toEqual(new OnsetDetector().detect(samples, SR))
    })
