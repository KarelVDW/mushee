...
import { InvalidShortcutsError, sanitizeStoredShortcuts } from '../../src/settings/stored-shortcuts'
...
describe('sanitizeStoredShortcuts', () => {
    ...
    it('rejects malformed shortcut ids', () => {
        for (const keys of ['', 'Bogus+KeyJ', 'KeyJ+Ctrl', 'Ctrl+', 'Ctrl+Key J', 'Ctrl+KeyJ;DROP']) {
            expect(() => sanitizeStoredShortcuts({ version: 1, overrides: { copy: { keys } } })).toThrow(InvalidShortcutsError)
        }
    })

    it('rejects malformed overrides and labels', () => {
        expect(() => sanitizeStoredShortcuts({ version: 1, overrides: { copy: 'KeyC' } })).toThrow(InvalidShortcutsError)
        expect(() =>
    ...
    it('caps the number of overrides and the command id length', () => {
        ...
        expect(() => sanitizeStoredShortcuts({ version: 1, overrides: { ['x'.repeat(101)]: { keys: 'KeyA' } } })).toThrow(
