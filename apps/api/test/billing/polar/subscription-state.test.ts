...
import { isEntitledStatus, patchFromCustomerState, patchFromSubscription } from '../../../src/billing/polar/subscription-state'

const resolveTier = (productId: string) => ({ 'prod-pro': 'pro', 'prod-studio': 'studio' })[productId] ?? null
...
describe('subscription lifecycle → row patch', () => {
    ...
    it('keeps a scheduled cancellation on the paid tier until period end', () => {
        const patch = patchFromSubscription({ ...baseSub, status: 'active', cancelAtPeriodEnd: true }, resolveTier)
    ...
    it('ignores products that are not ours', () => {
        expect(patchFromSubscription({ ...baseSub, productId: 'foreign', status: 'active' }, resolveTier)).toBeNull()
    })
})
...
describe('customer.state_changed reconciliation', () => {
    ...
    it('downgrades a paid tier when no active subscription remains', () => {
        const patch = patchFromCustomerState({ id: 'cus-1', activeSubscriptions: [] }, resolveTier, 'pro')
    ...
    it('never touches beta or free users without subscriptions', () => {
        ...
        expect(patchFromCustomerState({ id: 'cus-1', activeSubscriptions: [] }, resolveTier, undefined)).toBeNull()
    })
})
