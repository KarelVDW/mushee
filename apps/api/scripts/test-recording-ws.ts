...
async function startServer(): Promise<() => Promise<void>> {
    ...
    wss.on('connection', (client: WebSocket) => {
        ...
        client.on('message', (data: RawData, isBinary: boolean) => {
            ...
            try {
                parsed = JSON.parse(toBuffer(data).toString('utf8')) as RecordingControlMessage
            } catch {
...
async function runClient(audio: Buffer): Promise<{ measures: Record<number, MxmlMeasure> }> {
    ...
    ws.on('message', (data: Buffer) => {
        ...
        try {
            ...
            if (p.type === 'score-update') {
                ...
                console.log(`[client] score-update #${updatesReceived} measures=[${indices.join(',')}]`)
                if (p.measures) {
    ...
    ws.on('close', (code, reason) => console.log(`[client] closed ${code} ${reason.toString()}`))
...
async function main(): Promise<void> {
    ...
    try {
        ...
        console.log(`[client] accumulated ${Object.keys(measures).length} measures`)
    } finally {
        await stopServer()
