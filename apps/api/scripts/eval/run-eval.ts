...
import { defaultEvalOptions, type EvalReport, REAL_ROOT, runEval, SYNTH_ROOT } from './lib/evalRun'
...
export function printReport(report: EvalReport): void {
    ...
    for (const s of report.perScenario) {
        if (!s.clips) continue
        console.log(
            s.scenario.padEnd(30) +
                s.f1.toFixed(2).padEnd(7) +
                s.f1Off.toFixed(2).padEnd(9) +
    ...
    for (const m of report.perMaterial) {
        console.log(
            m.material.padEnd(20) +
                m.f1.toFixed(3).padEnd(7) +
                m.f1Off.toFixed(3).padEnd(9) +
                m.precision.toFixed(2).padEnd(7) +
                m.recall.toFixed(2).padEnd(8) +
                (m.provisional
                    ? m.datasets.length
            ...
            `† excluded from the aggregates: ${ex.excludedFromOverall.join(', ')}. ` +
                [
                    ...
                    constructed.length
                        ...
                        : '',
                    pitchless.length ? `Pitchless (${pitchless.join(', ')}) — onset-only truth; read the COn and COnRec columns.` : '',
                ]
                    .filter(Boolean)
                    .join(' ') +
    ...
    console.log(
        'dataset'.padEnd(30) + 'bias'.padEnd(8) + 'median'.padEnd(8) + 'std'.padEnd(8) + 'mae'.padEnd(8) + 'offBias'.padEnd(9) + 'n',
    )
    for (const s of report.perScenario) {
        if (!s.clips) continue
        const t = s.timing
