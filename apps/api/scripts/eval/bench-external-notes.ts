...
function main(): void {
  ...
  for (const file of readdirSync(EXT_DIR).sort()) {
    ...
    const raw = JSON.parse(readFileSync(join(EXT_DIR, file), 'utf8')) as [
      number,
      number,
      number,
    ][];
  ...
  const rec = (c: { matched: number; total: number }): string =>
    `${(c.total ? c.matched / c.total : 0).toFixed(3)}(${c.total})`;
  console.log(
    `onset recall by class: reOn=${rec(classStats.reonset)} ` +
      `trans=${rec(classStats.transition)} sil=${rec(classStats.silence)}`,
  );
}
