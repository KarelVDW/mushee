...
async function main(): Promise<void> {
    ...
    const VARIANTS = ['seg', 'clean', 'quant@120', 'quant@truthBpm', 'quant@ioi', 'quant@fit *', 'quant@oracle *', 'noquant *'] as const
    ...
    for (const ds of datasets) {
        ...
        for (const clip of clips) {
            ...
            record(rows['quant@truthBpm'], ref, toEst(extractor.quantize(cleaned, c.truth.bpm || 120)), c.truth.bpm)
            ...
            record(rows['quant@fit *'], ref, toEst(shifted(extractor.quantize(shifted(cleaned, -fitPhase), fitBpm), fitPhase)), fitBpm)
            ...
            for (let bpm = BPM_LO; bpm <= BPM_HI; bpm += 2) {
                ...
                for (let p = 0; p < PHASE_STEPS; p += 1) {
                    const phase = (p / PHASE_STEPS) * beatSec
                    const est = toEst(shifted(extractor.quantize(shifted(cleaned, -phase), bpm), phase))
        ...
        console.log(
            'variant'.padEnd(16) + TOLS.map((t) => `COnP@${t}`.padEnd(10)).join('') + 'seqF1'.padEnd(8) + 'est/ref'.padEnd(10) + 'bpm',
        )
