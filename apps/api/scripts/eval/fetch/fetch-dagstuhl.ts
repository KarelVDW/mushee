...
interface BeatMark { timeSec: number; beat: number }
...
function main(): void {
  ...
  interface Stem {
    file: string; piece: string; quartet: string; take: string;
    section: string; singer: string; mic: string;
  }
  const stems: Stem[] = [];
  for (const e of entries) {
    ...
    if (!m) continue;
    stems.push({
      file: base, piece: m[1], quartet: m[2], take: m[3],
      section: m[4], singer: m[4] + m[5], mic: m[6],
    });
  }
  ...
  for (const s of chosen) {
    ...
    for (let i = 0; i < MAX_EXCERPTS_PER_STEM; i += 1) {
      ...
      const beatAtStart = grid[0].beat +
        ((0 - grid[0].timeSec) / (grid[1].timeSec - grid[0].timeSec)) *
          (grid[1].beat - grid[0].beat);
