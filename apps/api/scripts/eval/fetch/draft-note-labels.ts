 ...
 */
const DATASET_OVERRIDES: Record<string, { track?: Partial<typeof TRACK>; segment?: Partial<typeof SEGMENT> }> = {
    'whistle-vintage': {
...
function main(): void {
    ...
    for (const ds of readdirSync(STAGE_ROOT, { withFileTypes: true })) {
        ...
        const staging = existsSync(join(stageDir, 'staging.json'))
            ...
            : {}
        const provenanceFor = new Map((staging.clips ?? []).map((c) => [c.id, c] as const))
        ...
        for (const file of readdirSync(stageDir)
            ...
            console.log(
                `  ${ds.name}/${clip}: ${notes.length} draft notes, ` +
                    ...
                    (pitches.length ? `${noteName(Math.min(...pitches))}–${noteName(Math.max(...pitches))}, ` : '') +
