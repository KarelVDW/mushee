...
function memberDataOffset(member: ZipMember): number {
    const head = fetchRange(zipUrl(member.zip), member.localHeaderOffset, 128, join(CACHE, 'zip', `${member.zip}.${member.entry}.lfh`))
...
function readAudioPrefix(member: ZipMember, neededSec: number): { pcm: Buffer; fmt: WavFormat } {
    ...
    for (const slack of [1.15, 1.4, Infinity]) {
        ...
        const want =
            slack === Infinity ? member.compressedSize : Math.min(Math.ceil(needed * ratio * slack) + HEAD_BYTES, member.compressedSize)
...
function pickSubset(candidates: Candidate[]): Candidate[] {
    const test = candidates.filter((c) => c.split === 'test').sort((a, b) => a.clip.localeCompare(b.clip))
...
function loadAnnotations(): Record<string, Annotation> {
    const dest = LOCAL_DIR ? join(LOCAL_DIR, 'annotations.json') : join(CACHE, 'annotations.json')
    if (!existsSync(dest)) {
        ...
        execFileSync(
            ...
            [
                ...
                '-o',
                `${dest}.part`,
                `${RECORD}/annotations.json/content`,
            ],
            { stdio: ['ignore', 'ignore', 'inherit'] },
        )
...
function main(): void {
    ...
    const stats = new Map<string, { clips: number; totalNotes: number; subjects: Set<number>; splits: Record<string, string> }>()
    for (const c of chosen) {
        ...
        const agg = stats.get(dataset) ?? { clips: 0, totalNotes: 0, subjects: new Set<number>(), splits: {} }
        agg.clips += 1
    ...
    for (const [dataset, agg] of Array.from(stats.entries())) {
        ...
        const manifest = {
            ...
            notes:
                ...
                `${
                    isTest
                        ? 'TEST half — keep it out of sweeps (SWEEP_EXCLUDE=n20emv2-test) and use it ' +
                          'only to confirm a decision already made'
                        : 'tunable half'
                }; per-clip origins are in ` +
                '`clipSplits`.',
        }
    ...
    console.log(`\nConverted ${clips} N20EMv2 clips (${totalNotes} notes) into ` + `${FIXTURES}/{${DEV_DATASET},${TEST_DATASET}}`)
