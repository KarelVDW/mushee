...
function buildClip(id: string, files: { path: string; midi: number }[], gapSec: number): Clip {
...
function main(): void {
    ...
    for (const inst of INSTRUMENTS) {
        for (const band of BANDS) {
            for (const dyn of DYNAMICS) {
                const pool = rows
                    .filter((r) => r.abbr === inst.abbr && r.dynamics === dyn && r.midi >= band.minMidi && r.midi <= band.maxMidi)
    ...
    extract([...new Set(plans.flatMap((p) => p.files.map((f) => f.path.slice(EXTRACT.length + 1))))])
    ...
    for (const inst of INSTRUMENTS) {
        ...
        for (const plan of list) {
            ...
            writeFileSync(join(out, `${clip.id}.truth.json`), `${JSON.stringify(clip.truth, null, 2)}\n`)
        ...
        console.log(`  ${inst.dataset}: ${list.length} clips, ${notes} notes, MIDI ${Math.min(...pitches)}–${Math.max(...pitches)}`)
