...
const ANNOT_URL = 'https://zenodo.org/api/records/6536490/files/JaCRC-annotations.zip/content'
const AUDIO_ZIP_URL = 'https://zenodo.org/api/records/6536490/files/JaCRC-recordings.zip/content'
...
function writeWavSlice(samples: Buffer, sampleRate: number, channels: number, startSec: number, endSec: number, dest: string): void {
...
function main(): void {
    ...
    const medianSyl = sylDurations.length ? sylDurations[Math.floor(sylDurations.length / 2)] : 0
