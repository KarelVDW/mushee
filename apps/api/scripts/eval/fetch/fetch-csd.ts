...
const ZIP_URL = 'https://zenodo.org/api/records/2649950/files/ChoralSingingDataset.zip/content'
...
function pickWindows(notes: TruthNote[]): number[] {
    ...
    for (let s = 0; s + WINDOW_SEC <= span; s += 5) {
        const count = notes.filter((n) => n.onsetSec >= s && n.onsetSec + n.durSec <= s + WINDOW_SEC).length
