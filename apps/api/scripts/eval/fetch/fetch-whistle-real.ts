...
const MELODY_DETECTION_RAW = 'https://raw.githubusercontent.com/ebezzam/melody-detection/master/wav_files'
...
const CLIPS: ClipSpec[] = [
    ...
    ...(['ppA', 'ppAfast', 'ppAslow', 'ppAup', 'ppB', 'ppBdown', 'ppBslow'] as const).map(
        (name): ClipSpec => ({
            id: `mit-${name}`,
            dataset: 'whistle-real',
    ...
    ...([0, 45, 90] as const).map(
        (startSec, i): ClipSpec => ({
            ...
            source: 'https://commons.wikimedia.org/wiki/File:Alice_J._Shaw_and_her_daughters_whistling.mp3',
            note: 'accompanied art whistling, 134 s source, median f0 ~1.48 kHz',
        }),
    ),
    ...([10, 55, 100] as const).map(
        (startSec, i): ClipSpec => ({
            id: `stafford-${i + 1}`,
            dataset: 'whistle-vintage',
            ...
            durSec: 30,
            licence: 'pd',
            attribution: 'Frank Stafford, "Der Spottvogel" (Septimus Winner, 1827–1902), public domain',
...
function curl(url: string, out: string): void {
    execFileSync('curl', ['-sL', '--fail', '--max-time', '600', '-A', UA, '-o', out, url], { stdio: ['ignore', 'ignore', 'inherit'] })
}
...
function stage(clip: ClipSpec): StagedClip | undefined {
    ...
    if (clip.commonsTitle) {
        ...
        if (!licenceAllowed(licence)) {
            console.warn(`  ⛔ ${clip.id}: Commons reports licence "${licence}", which is not in the allowlist — SKIPPED`)
            return undefined
        }
...
function apiGet(url: string, token: string): unknown {
    const raw = execFileSync('curl', ['-sL', '--fail', '--max-time', '120', '-A', UA, '-H', `Authorization: Token ${token}`, url], {
        encoding: 'utf8',
    })
...
function searchFreesound(token: string, query: string, wanted: number): FreesoundHit[] {
    ...
    for (let page = 1; hits.length < wanted; page += 1) {
        ...
        try {
            body = apiGet(`https://freesound.org/apiv2/search/text/?${params.toString()}`, token) as typeof body
        } catch {
...
function stageLocal(dir: string): StagedClip[] {
    ...
    for (const f of readdirSync(dir)
        .filter((n) => /\.(wav|webm|m4a|mp3|ogg)$/i.test(n))
        .sort()) {
        const id = `dogfood-${basename(f)
            .replace(/\.[^.]+$/, '')
            .replace(/[^a-z0-9-]+/gi, '-')}`
        const out = join(outDir, `${id}.wav`)
...
function main(): void {
    ...
    if (token) {
        ...
        console.log('  (no Freesound key in FREESOUND_TOKEN or scripts/eval/.freesound-token — ' + 'skipping the CC0 sweep)')
    }
    ...
    for (const [dataset, clips] of byDataset) {
        ...
        writeFileSync(join(dir, 'staging.json'), `${JSON.stringify({ dataset, sampleRate: SAMPLE_RATE, clips }, null, 2)}\n`)
