...
function pickWindow(all: TruthNote[], durationSec: number): Window | null {
  ...
  for (const anchor of all) {
    ...
    const inWindow = all.filter(
      (n) => n.onsetSec >= startSec && n.onsetSec <= endSec - TAIL_SEC,
    );
...
function main(): void {
  ...
  console.log(
    `  ${candidates.length} excerpts have a monophonic ${WINDOW_SEC} s window ` +
      `(${noWindow} rejected: too chordal or too sparse)`,
  );
