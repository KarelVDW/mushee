 ...
 */
async function runRealCorpus(registry: ProviderRegistry, resolver: ProfileResolver): Promise<void> {
    ...
    console.log(`${'dataset'.padEnd(22)}${'input'.padEnd(7)}` + `${'F1@.1'.padEnd(8)}${'F1@.2'.padEnd(8)}${'onsetBias'.padEnd(11)}clips`)
    for (const ds of datasets) {
        ...
        for (const label of codecs) {
            ...
            const t = timingStats(
                b.m.flatMap((m) => m.timing.onsetDeltasMs),
                b.m.flatMap((m) => m.timing.offsetDeltasMs),
            )
            console.log(
...
async function main(): Promise<void> {
    ...
    for (const id of scenarioIds) {
        ...
        for (const [label, audio] of [
            ...
            const est = await runThroughPipeline(registry, resolver, audio, truth.bpm, beats, scenario.instrumentId ?? '')
