...
function loopToWebm(wav: Buffer, n: number): Promise<Buffer> {
    ...
    return new Promise<Buffer>((res, rej) => {
        ...
        proc.on('error', rej)
        proc.on('close', (code) => (out.length ? res(Buffer.concat(out)) : rej(new Error(`loop failed (${code})`))))
        proc.stdin.on('error', () => {})
        proc.stdin.end(wav)
    })
}
...
async function main(): Promise<void> {
    ...
    const truth = JSON.parse(readFileSync(join(EVAL_ROOT, id, 'tune.truth.json'), 'utf8')) as GroundTruth
    ...
    await runThroughPipelineStreaming(registry, resolver, webm, truth.bpm, 4, scenario?.instrumentId ?? '', { chunks, chunkDelayMs })
