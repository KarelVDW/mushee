...
async function main(): Promise<void> {
  ...
  for (const ds of discoverRealDatasets(REAL_ROOT).filter((d) => DATASETS.includes(d.id))) {
    const clips = listRealClips(ds.dir).filter((c) => inSplit(ds.id, c, split)).slice(0, PER_DATASET);
    const plain: number[] = [];
    const lead: number[] = [];
    ...
    console.log(
      ds.id.padEnd(20) + String(clips.length).padEnd(4) + mean(plain).toFixed(3).padEnd(18) +
        mean(lead).toFixed(3).padEnd(20) + (mean(lead) - mean(plain) >= 0 ? '+' : '') + (mean(lead) - mean(plain)).toFixed(3),
    );
  }
  console.log(
    '\nALL'.padEnd(21) + String(all.length).padEnd(4) + mean(all.map((x) => x.plain)).toFixed(3).padEnd(18) +
