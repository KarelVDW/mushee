...
function applyQuorum(
  ...
  for (let i = 0; i < track.frames; i += 1) {
    raw[i] =
      track.confidence[i] >= gate.confidenceThreshold &&
      track.cents[i] >= lo &&
      track.cents[i] <= hi
        ? 1
        : 0;
...
function run(c: CachedClip, cfg: Config): EstNote[] {
  const gate = {
    confidenceThreshold: Math.min(
      0.95,
      Math.max(0.05, (c.profile.confidenceThreshold ?? 0.5) + (cfg.gateDelta ?? 0)),
    ),
...
async function main(): Promise<void> {
  ...
  const onlyParts = only?.split(',').map((s) => s.trim()).filter(Boolean);
  const selected = CONFIGS.filter(
    (c) =>
      c.name === 'SHIPPED' ||
      !onlyParts ||
      onlyParts.some((p) => c.name.includes(p)),
  );
  ...
  for (const cfg of selected) {
    ...
    for (const s of strata) {
      ...
      const cmp = pairedDiffCI(idx.map((i) => base[i]), idx.map((i) => f1s[i]));
