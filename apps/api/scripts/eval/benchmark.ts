...
async function run(): Promise<void> {
  ...
  const conditions =
    list(arg('conditions')) ?? (quick ? ['real'] : conditionsFor(true).map((c) => c.id));
...
function renderHistory(results: BenchResult[]): string {
  ...
  lines.push('| date (UTC) | commit | label | conditions | overall COnP | singing | humming | whistling | instrument | vocal-perc. COn | file |');
  lines.push('|---|---|---|---|---|---|---|---|---|---|---|');
...
function compare(aPath: string, bPath: string): void {
  ...
  const row = (name: string, ps: Pair[]): void => {
    if (!ps.length) return;
    const ci = pairedDiffCI(ps.map((p) => p.a.f1), ps.map((p) => p.b.f1));
    const ciOff = pairedDiffCI(ps.map((p) => p.a.f1Off), ps.map((p) => p.b.f1Off));
    const meanA = ps.reduce((s, p) => s + p.a.f1, 0) / ps.length;
    const meanB = ps.reduce((s, p) => s + p.b.f1, 0) / ps.length;
    console.log(
      name.padEnd(28) + `${meanA.toFixed(3)} → ${meanB.toFixed(3)}  ` +
        formatComparison(ci).padEnd(52) + `COnPOff ${ciOff.point >= 0 ? '+' : ''}${ciOff.point.toFixed(3)}${ciOff.significant ? '*' : ''}`,
    );
  };

  console.log('--- by material (clip-level pairs; the RESULTS.md headline is a dataset-mean, so read direction + significance here, not the level) ---');
  for (const m of MATERIAL_ORDER) row(m, pairs.filter((p) => p.material === m));
  row('ALL', pairs);

  console.log('\n--- by dataset ---');
  for (const d of [...new Set(pairs.map((p) => p.d))].sort()) row(d, pairs.filter((p) => p.d === d));

  const conds = [...new Set(pairs.map((p) => p.cond))];
  if (conds.length > 1) {
    console.log('\n--- by condition ---');
    for (const c of conds) row(c, pairs.filter((p) => p.cond === c));
  }
...
function resolveResult(p: string): string {
  ...
  const hit = existsSync(RESULTS_DIR)
    ? readdirSync(RESULTS_DIR).find((f) => f.includes(basename(p)))
    : undefined;
