...
| `fetch/fetch-humtrans.ts` | HumTrans (ICASSP 2024, **CC BY-NC 4.0 → `licenceRestricted`, context tier only**) — real HUMMING: 10 hummers, 44.1 kHz, reference MIDI per segment. Pulls only the corpus's own test split (769 segments) out of the 14.7 GB archive by byte range (`lib/remoteZip.ts`). Truth is the reference melody (exact identity, nominal timing); run `fetch/align-prescribed-truth.ts --dataset=context/humtrans --out=humtrans-aligned` with `ALIGN_MIN_HZ=70 ALIGN_MAX_HZ=1200 ALIGN_OCTAVE_INVARIANT=1` for performance-aligned onsets. `HUMTRANS_SPLIT`, `HUMTRANS_MAX`. |
