...
| `fetch/triage-verify-worklist.ts` | Refresh the "lowest pipeline agreement" section of a dataset's `annotations/<dataset>/VERIFY-WORKLIST.md` from the latest committed benchmark result — the clips where pipeline and draft disagree most are where a human-verified label changes a number either way. |
...
| `fetch/fetch-humtrans.ts` | HumTrans (ICASSP 2024, **CC BY-NC 4.0 → `licenceRestricted`, context tier only**) — real HUMMING: 10 hummers, 44.1 kHz, reference MIDI per segment. Pulls only the corpus's own test split (769 segments) out of the 14.7 GB archive by byte range (`lib/remoteZip.ts`). Truth is the reference melody (exact identity, nominal timing); run `fetch/align-prescribed-truth.ts --dataset=context/humtrans --out=humtrans-aligned` with `ALIGN_TRACKER=yin ALIGN_MIN_HZ=70 ALIGN_MAX_HZ=1000 ALIGN_MIN_TONALITY=0.5` for performance-aligned onsets (and the octave the hummer actually used — four of the five men hum the reference an octave down). `HUMTRANS_SPLIT`, `HUMTRANS_MAX`. |
...
| `fetch/align-prescribed-truth.ts` | Repair a "performed to a prescribed melody" dataset into performance truth: note identity from the score (key/octave detected per clip), onsets from the audio, DTW-aligned. Output is a `-aligned` sibling, still `noteTruthDerived`. `ALIGN_MIN_HZ`/`ALIGN_MAX_HZ`/`ALIGN_OCTAVE_INVARIANT` move the drafter to the hummed register. |
