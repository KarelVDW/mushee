...
| -------------------------------------------------------------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Replace CREPE-tiny with SwiftF0 / HarmoF0 / RMVPE for range and robustness | **DISCARDED** (measured)                 | `bench-pitch-models.ts` + `bench_pitch_models.py`: SwiftF0 ties overall, −0.17 mid band, worse under degradation; HarmoF0 sub-octave from ~1.2 kHz; RMVPE barred (NC weights). SwiftF0 remains a _cost_ option (5.6× cheaper CPU) |
...
measured on a variant cache built _without_ the shipped reverberance relief
...
otherwise). Open and unaddressed: the resolver locks onto the _accompaniment_
...
_provisional_ humming row. A benchmark-grade row still needs permissively
