...
7. **There is no commercially usable note-annotated humming corpus, at all.** HumTrans (the only
   real candidate, 56 h) is CC-BY-NC-4.0 _and_ its onset/offset ground truth is independently
...
| **MIR-QBSH**                           | 4,431 clips, ~195 subjects, 8 kHz/8-bit mono; 48 reference MIDIs               | **frame-level pitch only** (`.pv`, 256-sample frames, semitone units). Self-labelled by student recordists; "no guarantee for their correctness"   | **NONE STATED** — citation request only, worded "for your research" [verified from bundled `index.htm`]                                                      | ⚠️ **NO / unclear** — no commercial grant                                             | ✅ [mirlab.org](http://mirlab.org/dataset/public/) `MIR-QBSH.zip` (145 MB, HTTP only)                                                              | QBSH _retrieval_ (MRR/top-k) only; no note-transcription benchmark                                           |
...
| **Dagstuhl ChoirSet**                  | 108 tracks                                                                     | ⚠️ **NOT note-annotated** — see below. f0 (CREPE/pYIN-derived) + _score representation_ + beats; amateur multitrack choir, individual singer stems | **CC-BY-4.0** [verified: [Zenodo 4618287](https://zenodo.org/records/4618287)]                                                                               | ✅ licence yes, ⛔ **unusable as note truth**                                         | ✅ Zenodo                                                                                                                                          | ❌ none                                                                                                      |
...
| **Opencpop**                           | 100 Mandarin songs, **5.2 h**, 3,756 utterances, 1 pro female singer, 44.1 kHz | utterance/**note**/phoneme boundaries + note pitch + slur labels                                                                                   | **CC-BY-NC-**ND**-4.0** — NC _and_ NoDerivatives                                                                                                             | ❌ **NO** by default; **commercial licence obtainable by email** (zpcoftts@gmail.com) | 🔑 Google Form → email                                                                                                                             | —                                                                                                            |
...
| **PTDB-TUG**                           | **9.6 h**, 20 native-EN speakers, 4,720 TIMIT sentences, 3.9 GB                | **f0 only** — laryngograph-derived (physiological, not hand-labelled). **Speech, not singing**                                                     | **ODbL 1.0** + DbCL 1.0 — §3.1: _"These rights explicitly include commercial use, and do not exclude any field of endeavour."_ [verified]                    | ✅ **YES**                                                                            | ✅ `www2.spsc.tugraz.at/databases/PTDB-TUG`                                                                                                        | ✅ best-documented f0 leaderboard (§2.5)                                                                     |
...
| **JVS-MuSiC**                          | 100 singers × 2 songs, 24 kHz, 0.6 GB                                          | **no note annotations**; Melodyne project files + similarity/key/tempo tags                                                                        | tags CC-BY-SA-4.0; audio _"non-commercial research"_, **"re-distribution is not permitted"**                                                                 | ⚠️ **treat as NO**                                                                    | ✅ Drive                                                                                                                                           | —                                                                                                            |
...
**The systematic pattern.** Every note-annotated _singing_ corpus other than vocadito and
...
**Worse, HumTrans's note ground truth is known-broken.** The follow-up paper _Dynamic HumTrans_
...
[repo](https://github.com/shubham-gupta-30/humming_transcription) is blunter: _"the dataset we
are working with has incorrect onsets and offsets as ground truth"_ — they had to correct onsets
...
The likely root cause is the collection design: subjects hummed _while reading a score with the
melody audio playing_, so the "ground truth" is the **reference score**, not an annotation of
...
- **The vocal datasets have been withdrawn.** [ccrma.stanford.edu/damp](https://ccrma.stanford.edu/damp/)
  states verbatim: _"The DAMP vocal datasets are no longer available for download."_ Only Magic
  Piano performance data remains.
- **The license was never commercial-compatible anyway.** Per the same page, Smule's Research Data
  License Agreement required that _"the data could only be used for non-commercial, research
  purposes"_, that it _"could not be shared with anyone who did not also accept the terms"_, and
  that it _"could not be re-hosted in any way. This includes hosting on either a public website or
  repository, or private server"_ — the last clause alone would forbid our fixture cache.
...
**But the good news, and it's better than expected: the two corpora we already use _are_ our
...
- **vocadito is deliberately consumer-device audio.** From the paper §2.1, verbatim: _"In order to
  simulate a 'real-world' setting, we did not restrict volunteers to record using high-quality
  microphones, and **many of the recordings are from cell phone or computer microphones**."_ 28
  volunteers of varying singing experience, self-chosen original/public-domain songs, 10–40 s each,
  and _"volunteers agreed to their recordings being anonymously included in this dataset and
  publicly released"_ — a clean consent model we can copy for T4. **CC-BY-4.0.** This makes
...
<1K real recorded _heavily degraded singing_ clips, and it ships **`pairwise-ranking.csv` plus the
...
Molina et al. 2014) — never publicly released, and its ground truth was _the original score_,
...
**Consequence:** a _perfect_ f0 tracker fed through our MIR-QBSH label pipeline would score
...
_smoke/robustness_ corpus and never as a gate.
...
Molina, Barbancho, Tardón & Barbancho, _Evaluation Framework for Automatic Singing
Transcription_, ISMIR 2014 ([PDF](https://archives.ismir.net/ismir2014/paper/000298.pdf)) —
...
- **COnP** — onset + pitch, offset ignored. _This is what our harness computes._
- **COn** — onset only.
...
tested**, and they concluded that _"onset detection within a range of ±50 ms is very restrictive
in the case of singing voice with lyrics, since many onsets are not clear even for an expert
musician."_ That is the strongest published justification for a wider tolerance on sung input —
...
pitch _"within a half semitone (+/- 3%)"_, onset _"within a 100ms range (+/- 50ms)"_, offset
_"within 20% range of the ground truth note's offset"_. Note MIREX phrases the onset tolerance as
...
2. **Exact-MIDI vs 50 cents.** We round to integer MIDI and require equality; `mir_eval`
   compares continuous pitches within 50 cents. These coincide _only if_ the reference is also
...
- **We default to `VOCADITO_ANNOTATOR=A1`**, the _fine-grained_ annotator. The paper's own
  ...
  notation product) merges ornaments into one note. The authors' explicit recommendation: _"if
  any style of transcription is acceptable, we recommend reporting the scores as in Amax."_
...
- **Any vocadito target above ~0.76 is meaningless.** Express our score as a _fraction of the
  human ceiling_, not an absolute.
...
Caveat: M4Singer is _professional_ singers reading composed scores, so 77.4 is an easier
...
**f0 tracking on PTDB-TUG** (licence-clean, ODbL) — _Cross-domain Neural Pitch and Periodicity
Estimation_, [ar5iv 2301.12258](https://ar5iv.labs.arxiv.org/html/2301.12258), 50-cent tolerance:
...
separation (Mel-RoFormer _is_ a separation model). Our task is **solo, dry, consumer-mic voice /
...
singers on cell-phone mics produce audio whose _human annotator agreement_ (0.64–0.74) is below
the _machine COnP_ on professional corpora (0.80–0.86). Any target we set must be corpus-specific;
...
- **Always compare paired** (same clips, both versions) and report the mean _difference_ with a
...
picks a winner _by test-set F1_ transfers a little information from the test set into the
...
Ycart, Liu, Benetos & Pearce, _Investigating the Perceptual Validity of Evaluation Metrics for
Automatic Piano Music Transcription_, **TISMIR 3(1):68–81, 2020**
...
- **They explicitly contradict the received wisdom** that onset+offset F is the most
  perceptually relevant metric: _"the OnOff-Note metric, presented as the most
  perceptually-relevant evaluation metric by Hawthorne et al. (2018), is actually not the best
  metric in terms of agreement with human ratings."_ They keep it only as "a meaningful objective
...
- **F-measure is unreliable exactly where we operate.** When ΔF > 50 % raters always agree with
  ...
  _"F_n,On is a good enough metric in clear-cut cases … but should probably be treated with
  caution for small differences between AMT systems … very often, differences between systems are
  of the order of a few percentage points."_ **This is the regime all our tuning work lives in.**
...
- **Rhythm features were the most valuable non-benchmark contribution** in their ablation.
  Octave-error and out-of-key features were _counter-productive_ in their linear model — keep
...
- Two scope limits the authors state, both relevant: (i) _"should not be generalised e.g. to
  singing voice"_ — so treat the 75–150 ms finding as strong but not proven for our domain;
  (ii) perceptual similarity is the right criterion _"when the overall musical quality … matters
  more than precise transcription of every note, e.g. quick dictation of musical ideas"_ — which
...
McLeod & Steedman, _Evaluating Automatic Polyphonic Music Transcription_, ISMIR 2018
...
- **MUSTER** (Nakamura et al., ICASSP 2018 / ISMIR 2021; tool at
  ...
  `E_p` pitch, `E_e` extra, `E_m` missing, `E_on` onset (via a _rhythm correction cost_ = minimum
  scale/shift operations on onset score times), `E_off` offset, `E_v` voice, and `E_all` as their
  mean. Their argument, quotable: _"The edit-distance-based metrics have a clear interpretation:
  they count how many notes or score elements should be edited to obtain the correct score."_
- **Sheet Music Benchmark** (ISMIR 2025, [arXiv:2506.10488](https://arxiv.org/html/2506.10488v1))
  on Symbol Error Rate: _"it still stands as the best metric to correlate the human correction
  effort that is necessary to obtain the desired score. However such correction effort does not
  indicate in which aspect of the transcription process the OMR is struggling."_ — i.e. pair a
...
- Román et al. (ISMIR 2018, [PDF](http://ismir2018.ircam.fr/doc/pdfs/87_Paper.pdf)), the one
  **monophonic** audio-to-score precedent, used ASR-style **CER 5.36 % / WER 15.67 %** and found
  _"the majority of errors are due to wrong time signatures, barline locations, and clefs"_ — a
...
MIREX QBSH scoring is _"Mean reciprocal rank (MRR) of the ground truth, as well as the simple
hit(1)/miss(0) counting … over the top 10 returns"_
...
But it is structurally blind to the errors that break a _score_: Molina et al. note directly that
_"frame-based systems for query-by-humming are not affected by splits."_ QBH matches pitch
...
competitor has invited; and _correction effort_ is both the honest metric and the competitive axis.
...
- **Adopt Molina et al.'s annotation conventions** for any singing ground truth we create
  ourselves: ornaments and vibrato are _not_ separate notes; portamento does _not_ create a third
...
- **AGC and browser noise suppression are on by default** (`getUserMedia`
  `autoGainControl: true`, and WebRTC's APM runs spectral NS). AGC is a _time-varying_ envelope:
...
- **SSVD v2.0 is the one tempting unknown**: 194 sight-singing recordings with _fully manual_
...
- **The "±100 ms is perceptually better" evidence is piano-only.** Ycart et al. state explicitly
  that their results _"should not be generalised e.g. to singing voice."_ The 75–150 ms finding is
...
- **ISMIR2014/Molina dataset — ⛔ CLOSED, 2026-08-08: it is NON-COMMERCIAL, and that was never
  the question anyone asked.** Every previous note in these docs framed this as an _availability_
  problem ("dead URL", "mirror unconfirmed", "worth an email"). It is a _licence_ problem, and the
  ...
  wrong), and its `readme.txt`, recovered from the Wayback Machine, states:
    ...
    data" rule bars it outright. **Stop chasing it.** Note the split: the _toolbox_ (CommandLineTool
    ...
    MTG-QBH is a query-by-humming _retrieval_ corpus — song-identity metadata, no note-level truth,
...
- **Dagstuhl ChoirSet is NOT a note corpus — verified against the artifact, 2026-08-08.** The row
  ...
  There is **no performed-note annotation anywhere in it**: the "notes" are a _score_
...
- **Realistic-user-corpus practice** (crowdsourcing mechanics, GDPR posture, nightly-eval
  ...
  question myself (§1.2b — withdrawn _and_ non-commercial), but **GDPR lawful basis and retention
...
| **T5 human**   | 2AFC panel, new build vs production, + difficulty question                   | 150–300 judgements | per release, and for any sub-10-pt F change | is it actually _better_                    | win-rate CI excludes 50 %                 |
...
2. **COnPOff F1 @ 50 ms**, offset tol `max(50 ms, 20 %·dur)` — currently missing entirely.
   Track it as an engineering objective (it is what separates a correct _note value_ from a
...
4. **Chroma-F1 and octave-error rate** — keep as **diagnostics**. CREPE's RPA≈RCA on vocadito
   ...
   counter-productive as _score components_ — so diagnose with them, don't optimise them.
...
- **Do target the ROSVOT/M4Singer numbers** (COnPOff 0.774, COnP 0.803, COn 0.940) for our
  _clean professional_ tier — same task shape (monophonic solo, no separation).
...
- **Never ship a decision on a sub-10-point F difference on automatic metrics alone.** Ycart et
  ...
  bootstrap CI can confirm a +2 pt change is _real_ while saying nothing about whether it is
  _better_. For changes in that band, route to the T4 human 2AFC panel before shipping. This is
...
7. **Add "notes to fix" (edit distance) and the T5 human 2AFC panel.** These are what make the
   benchmark _product_-facing rather than research-facing.
