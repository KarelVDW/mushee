...
export function segErrors(
  ...
  const counts: SegErrorCounts = {
    ...
    tangled: 0, pitchWrong: 0, refTotal: ref.length, estTotal: est.length,
  };
  ...
  for (let i = 0; i < ref.length; i += 1) {
    ...
    const refIds: number[] = [];
    const estIds: number[] = [];
    const stack: Array<{ side: 'ref' | 'est'; id: number }> = [
      { side: 'ref', id: i },
    ];
    while (stack.length) {
...
export function repairSecondsPer100(c: SegErrorCounts): number {
  ...
  const total =
    c.split * REPAIR_SECONDS.join +
    c.merged * REPAIR_SECONDS.split +
    c.spurious * REPAIR_SECONDS.del +
    c.missed * REPAIR_SECONDS.create;
