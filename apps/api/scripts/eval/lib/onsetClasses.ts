...
export function addOnsetClassStats(into: OnsetClassStats, from: OnsetClassStats): void {
 ...
 */
export function classifyOnsets(ref: TruthNote[], gapSec = CONNECTED_GAP_SEC): OnsetClass[] {
 ...
 * pitch predicate.
 */
export function onsetRecallByClass(ref: TruthNote[], est: EstNote[], tolSec = 0.1): OnsetClassStats {
    const sorted = [...ref].sort((a, b) => a.onsetSec - b.onsetSec)
...
const pct = (c: ClassRecall): string => (c.total ? (c.matched / c.total).toFixed(3) : ' n/a ')
