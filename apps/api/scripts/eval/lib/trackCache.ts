...
import type { PitchProvider, PitchSession } from '../../../src/recordings/pipeline/providers/pitch-provider'
...
export function resolverEnvSignature(): string {
    const set = RESOLVER_ENV_KEYS.filter((k) => (process.env[k] ?? '') !== '').map((k) => `${k}=${process.env[k]}`)
...
export class TrackCache {
    ...
    private async compute(ds: RealDataset, clip: string): Promise<CachedClip | null> {
        ...
        const fineEnergy = this.onsetDetector.envelope(decoded.samples, provider.sampleRate)
        const fineHop = Math.max(1, Math.round(this.onsetDetector.hopSec * provider.sampleRate))
        const onsetTimesSec = this.onsetDetector.detectFromEnvelope(fineEnergy, fineHop, provider.sampleRate)
    ...
    private readFromDisk(ds: RealDataset, clip: string, truth: GroundTruth, metaPath: string, binPath: string): CachedClip | null {
...
interface TrajectoryProvider extends PitchProvider {
    track(samples: Float32Array, session?: PitchSession): Promise<PitchTrack | null>
}
