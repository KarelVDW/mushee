...
export class VariantTrackCache {
     ...
     */
    async load(ds: RealDataset, clip: string, variant: string, frontEnd?: AudioFrontEnd): Promise<CachedVariant | null> {
    ...
    private async compute(ds: RealDataset, clip: string, variant: string, frontEnd?: AudioFrontEnd): Promise<CachedVariant | null> {
        ...
        const det = frontEnd ? { ...rawDet, samples: frontEnd.apply(rawDet.samples, DETECT_SR) } : rawDet
        ...
        const onsetHop = Math.max(1, Math.round(this.onsetDetector.hopSec * provider.sampleRate))
        const envelope = this.onsetDetector.envelope(decoded.samples, provider.sampleRate)
        const onsetTimesSec = this.onsetDetector.detectFromEnvelope(envelope, onsetHop, provider.sampleRate)
        const energy = frameEnergy(decoded.samples, provider.sampleRate, track.hopSec, track.frames)
...
function frameEnergy(samples: Float32Array, sampleRate: number, hopSec: number, frames: number): Float32Array {
