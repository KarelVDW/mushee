...
function filterbank(): { starts: Int32Array; weights: Float32Array[]; bands: number } {
  ...
  for (
    let hz = MIN_BAND_HZ;
    hz <= MAX_BAND_HZ;
    hz *= Math.pow(2, 1 / BANDS_PER_OCTAVE)
  ) {
    centres.push(hz);
  }
  ...
  for (let b = 1; b < centres.length - 1; b += 1) {
    ...
    for (let k = from; k <= to; k += 1) {
      w[k - from] =
        k <= mid ? (k - lo) / Math.max(1e-9, mid - lo) : (hi - k) / Math.max(1e-9, hi - mid);
    }
    starts[bands] = from;
    weights.push(w);
    bands += 1;
...
const FB = filterbank();
const HANN = new Float32Array(FFT_SIZE).map(
...
export function splitAtSpectralPeaks<T extends NoteLike>(
  ...
  const isFlat = (n: NoteLike): boolean => {
    ...
    const to = Math.min(
      track.frames,
      Math.round((n.startTimeSeconds + n.durationSeconds) / track.hopSec),
    );
    let sum = 0;
    let sumSq = 0;
    let count = 0;
