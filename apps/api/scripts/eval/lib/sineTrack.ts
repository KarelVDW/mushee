 ...
 */
export function trackSinusoid(
  samples: Float32Array,
  sampleRate: number,
  opts: SineTrackOptions = {},
): SineFrame[] {
...
export function whistleScreen(samples: Float32Array, sampleRate: number): WhistleScreen {
  ...
  if (tonalFraction < MIN_TONAL_FRACTION) {
    return { keep: false, reason: `tonalFraction ${tonalFraction.toFixed(2)} < ${MIN_TONAL_FRACTION} (not one dominant partial)`, ...stats };
  }
