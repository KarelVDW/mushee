 ...
 */
export function pairedDiffCI(
  a: number[],
  b: number[],
  { resamples = DEFAULT_RESAMPLES, seed = 1, alpha = 0.05 } = {},
): PairedComparison {
  const n = Math.min(a.length, b.length);
  if (n === 0) {
    return {
      point: 0, low: 0, high: 0, sigma: 0, rho: 0, sigmaDiff: 0,
      mde: Infinity, n: 0, significant: false,
    };
  }
