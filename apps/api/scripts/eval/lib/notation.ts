...
export function toBeats(notes: { onsetSec: number; durSec: number; midi: number }[], bpm: number): BeatNote[] {
    const bps = bpm / 60
    return notes.map((n) => ({
...
export function truthToBeats(notes: TruthNote[], bpm: number, beatGrid?: { timeSec: number; beat: number }[]): BeatNote[] {
...
export class NotationScorer {
    ...
    private matchAt(ref: BeatNote[], est: BeatNote[], scale: number): NotationScore {
        ...
        for (const r of sortedRef) {
            ...
            if (Math.abs(snap(est[bestIdx].durBeat) - snap(r.durBeat)) <= this.durTolBeat + 1e-9) {
                durRight += 1
            }
        }
