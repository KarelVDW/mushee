 ...
 */
export function melodyToMidi(melody: Melody, rootMidi: number, program: number): Buffer {
    const events: number[] = []
    const microsPerQuarter = Math.round(60_000_000 / melody.bpm)

    // Tempo meta event (delta 0).
    events.push(...vlq(0), 0xff, 0x51, 0x03, (microsPerQuarter >> 16) & 0xff, (microsPerQuarter >> 8) & 0xff, microsPerQuarter & 0xff)
