...
export function degrade(inPath: string, outPath: string, condition: Condition, sampleRate = 44100, maxDurationSec?: number): void {
    ...
    if (condition.codec) {
        ...
        execFileSync(
            ...
            [
                ...
                tmpWav,
                '-af',
                `apad=pad_dur=${CODEC_TAIL_PAD_SEC}`,
                '-c:a',
                condition.codec.encoder,
                '-b:a',
                `${condition.codec.bitrateKbps}k`,
                '-ac',
                '1',
                tmpEnc,
            ],
