 ...
 */
export function synthesizeArticulated(truth: GroundTruth, opts: ArticulatedSynthOptions): Float32Array {
    ...
    for (let i = 0; i < notes.length; i += 1) {
        ...
        const legatoIn = prev !== null && shape.closureSec === 0 && startSec - (prev.onsetSec + prev.durSec) <= CONTIGUOUS_SEC
        const legatoOut = next !== null && shape.closureSec === 0 && next.onsetSec - endSec <= CONTIGUOUS_SEC
...
export function synthesize(truth: GroundTruth, opts: SynthOptions): Float32Array {
    ...
    for (const note of truth.notes) {
        ...
        for (let i = 0; i < nSamples; i += 1) {
            ...
            const noise = (rng() * 2 - 1) * (opts.kind === 'whistle' ? 0.012 : 0.02)
