...
const WINDOW = new Float32Array(FFT_SIZE).map((_, i) => Math.sqrt(0.5 - 0.5 * Math.cos((2 * Math.PI * i) / (FFT_SIZE - 1))))
 ...
 */
function solveComplex(aRe: Float64Array, aIm: Float64Array, bRe: Float64Array, bIm: Float64Array, k: number): void {
