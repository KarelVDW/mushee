...
const CONFIGS: Config[] = [
    ...
    {
        name: 'dip.7 rise1.4 ioi.07',
        seg: { ...SEMI },
        ext: { maxGridDivisor: 4 },
        bpm: 'gt',
        onset: { dipRatio: 0.7, riseRatio: 1.4, minIoiSec: 0.07 },
    },
    {
        name: 'dip.8 rise1.3 ioi.06',
        seg: { ...SEMI },
        ext: { maxGridDivisor: 4 },
        bpm: 'gt',
        onset: { dipRatio: 0.8, riseRatio: 1.3, minIoiSec: 0.06 },
    },
]
...
async function main(): Promise<void> {
    ...
    for (const ds of datasets) {
        const acc: Record<
            string,
            { f1: Record<number, number[]>; seq: number[]; seqP: number[]; seqR: number[]; estN: number[]; refN: number[] }
        > = {}
        ...
        for (const clip of listRealClips(ds.dir)) {
            ...
            for (const c of CONFIGS) {
                ...
                a.seqP.push(s.p)
                a.seqR.push(s.r)
                a.estN.push(est.length)
                a.refN.push(truth.notes.length)
            }
        }
