...
function segmentFor(
  ...
  return segmentNotes(cents, confidence, frames, {
    ...
    sampleRate: 1 / hopSec,
    confidenceThreshold:
      over.confidenceThreshold ?? profile.confidenceThreshold ?? 0.5,
...
const newScoreRow = (): ScoreRow => ({
  f1: [], precision: [], recall: [], estN: [], refN: [],
  onsetBias: [], offsetBias: [], estDur: [], refDur: [],
});
  ...
  for (const variant of variants) {
    ...
    console.log(
      '\n-- substitution ablation (stage=clean, F1@0.1 vs ground truth) --\n' +
        'variant'.padEnd(17) +
        'F1'.padEnd(8) + 'P'.padEnd(8) + 'R'.padEnd(8) +
        'est/ref'.padEnd(13) + 'medDur e/r'.padEnd(13) +
        'onsetMed'.padEnd(10) + 'offsetBias'.padEnd(11) + 'Δ vs deg',
    );
    ...
    for (const which of ['clean', 'deg'] as const) {
      ...
      const bins = edges.slice(0, -1).map((lo, i) =>
        ds.filter((d) => d >= lo && d < edges[i + 1]).length,
      );
    ...
    console.log(
      'shift'.padEnd(9) + `${variant} F1`.padEnd(13) + 'Δ'.padEnd(11) +
        'clean F1'.padEnd(11) + 'Δ clean',
    );
    ...
    const bestMs = shiftsMs.reduce(
      (a, b) => (mean(shiftScore(degEst, b)) > mean(shiftScore(degEst, a)) ? b : a),
      0,
    );
    if (bestMs > 0) {
      console.log(
        `  best shift ${bestMs}ms on ${variant}: ` +
          formatComparison(pairedDiffCI(degBase, shiftScore(degEst, bestMs))),
      );
      console.log(
        `  same shift on clean audio:       ` +
          formatComparison(pairedDiffCI(cleanBase, shiftScore(cleanEst, bestMs))),
      );
    }
    ...
    const agreements = pairs
      .map(frameAgreement)
      .filter((a): a is FrameAgreement => a !== null);
    console.log('\n-- frame level, degraded vs its OWN clean take --');
    console.log(
      `  voicing recall (clean-voiced frames still voiced) ${mean(agreements.map((a) => a.voicingRecall)).toFixed(3)}`,
    );
    console.log(
      `  false voicing (unvoiced->voiced, per frame)       ${mean(agreements.map((a) => a.falseVoicingRate)).toFixed(3)}`,
    );
    console.log(
      `  RPA@50c on frames voiced in both                  ${mean(agreements.map((a) => a.rpa)).toFixed(3)}`,
    );
    console.log(
      `  octave-error rate on those frames                 ${mean(agreements.map((a) => a.octaveRate)).toFixed(3)}`,
    );
    console.log(
      `  median |Δcents| on those frames                   ${mean(agreements.map((a) => a.medianAbsCents)).toFixed(1)}`,
    );
    console.log(
    ...
    console.log(
      `  snrDb      median clean ${median(pairs.map((p) => p.clean.scan.snrDb ?? NaN).filter(Number.isFinite)).toFixed(3).padStart(7)}` +
        ` -> degraded ${median(pairs.map((p) => p.deg.scan.snrDb ?? NaN).filter(Number.isFinite)).toFixed(3).padStart(7)}   ` +
        separability(
...
function productionConfThreshold(c: CachedVariant, lockSec?: number): number {
  ...
  const base = c.profile.id.includes('+reverb')
    ? UNRELAXED_CONFIDENCE
    : (c.profile.confidenceThreshold ?? UNRELAXED_CONFIDENCE);
  ...
  for (const p of pairs) {
    ...
    try {
      ...
      for (const sec of LOCK_PREFIXES_SEC) {
        locked?.set(
          sec,
          estimateReverberance(det.samples.subarray(0, Math.round(sec * 16000)), 16000),
        );
      }
    } catch {
...
function separability(
  ...
  const cut =
    direction === 'higher'
      ? s[Math.floor(0.9 * (s.length - 1))]
      : s[Math.floor(0.1 * (s.length - 1))];
  const hit =
    direction === 'higher'
      ? degVals.filter((v) => v > cut).length
      : degVals.filter((v) => v < cut).length;
...
async function sweep(
  ...
  for (const variant of [CLEAN_VARIANT, ...variants]) {
    ...
    const target = (p: ClipPair): CachedVariant =>
      variant === CLEAN_VARIANT ? p.clean : p.deg;
...
async function frontendSweep(
  ...
  for (const variant of [CLEAN_VARIANT, ...variants]) {
    ...
    for (const fe of fronts) {
      ...
      console.log(
        fe.id.padEnd(24) +
          mean(scores).toFixed(3).padEnd(8) +
          formatComparison(pairedDiffCI(kept, scores)),
      );
    }
  }
}
