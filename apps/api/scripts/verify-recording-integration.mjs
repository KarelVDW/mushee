                ...
                parts: [
                    {
                        ...
                        measures: [
                            {
                                number: '1',
                                entries: [{ _type: 'note', rest: { measure: true }, duration: 48, voice: '1', type: 'whole' }],
                            },
                        ],
                    },
                ],
...
async function main() {
    ...
    // 2. Missing scoreId.
    {
        ...
        check(
            ...
            c.messages.some((m) => m.type === 'recording-error' && m.code === 'score-required') && close.code === 1008,
    ...
    // 3. Score that isn't the user's.
    {
        ...
        check(
            ...
            c.messages.some((m) => m.type === 'recording-error' && m.code === 'score-not-found') && close.code === 1008,
            JSON.stringify({ close, messages: c.messages }),
        )
    }
    ...
    //    keeps it open so the take's last notes can still arrive.
    {
        ...
        const usage = await db.query('SELECT "creditsUsed" FROM recording_usage WHERE "userId" = $1', [userId])
        const used = usage.rows[0]?.creditsUsed ?? 0
        check('credits spent ≈ seconds recorded (3-5)', used >= 3 && used <= 5, `used=${used}`)

        const rec = await db.query('SELECT "scoreId", "creditsSpent", "endedAt" FROM recordings WHERE "userId" = $1', [userId])
    ...
    //    must not free the taken-over lock (token guard).
    {
        ...
        check(
            ...
            second.messages.some((m) => m.type === 'recording-error' && m.code === 'concurrent-recording') && secondClose.code === 1008,
        ...
        await db.query(`UPDATE active_recordings SET "heartbeatAt" = now() - interval '60 seconds' WHERE "userId" = $1`, [userId])
        ...
        check(
            ...
            fourth.messages.some((m) => m.type === 'recording-error' && m.code === 'concurrent-recording') && fourthClose.code === 1008,
    ...
    const freeLimit = (await db.query(`SELECT "dailyRecordingCredits" FROM subscription_tiers WHERE id = 'free'`)).rows[0]
        .dailyRecordingCredits
    {
        await db.query(`UPDATE recording_usage SET "creditsUsed" = $2 WHERE "userId" = $1`, [userId, freeLimit - 2])
        const c = connect(`/recording?scoreId=${scoreId}`, cookie)
        await c.open
        ...
        check(
            ...
            limitMsg?.planId === 'free' && limitMsg?.limitSeconds === freeLimit && limitMsg?.usedSeconds >= freeLimit,
